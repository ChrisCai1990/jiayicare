...
export default function WeightPhoto({ onSaved }) {
  ...
  const choose = async () => {
    ...
    try {
      ...
      setDraft(null); setConfirmed(false); setValue(''); setPhoto({ path, image }); setWhen(localTime());
  ...
  const submit = async () => {
    ...
    if (!Number.isFinite(number) || number <= 0 || number >= 500) { setError('请填写有效的体重数值'); return; }
    lock.current = true; setBusy(true); setError('');
    try {
      await recordsAPI.create({
        type: 'weight', category: 'vitals', label: '体重', unit: 'kg', value: String(number),
  ...
  return <View style={box}>
    ...
    <Text style={{ display: 'block', fontSize: '12px', marginTop: '8px' }}>仅支持单位明确为 kg 的体重秤屏幕照片；识别后核对确认，不会自动入档。</Text>
