...
test('首次代诊读取代诊日信息并完成检查预约', () => {
  ...
  for (const text of ['需要开具', '需向专家沟通', '专家诊疗意见及医嘱', '本次代诊实际开具的检验检查单/项目', '包括原计划项目和临时新增项目', '检验检查项目', '执行科室', '楼栋、楼层或具体地点', 'department', 'location', '检查安排', '检查后专家门诊', '各项检查可安排不同日期', '检查应安排在检查后专家门诊之前']) assert.match(`${proxy}\n${route}`, new RegExp(text));
});
...
test('陪诊完成后资料进入报告审核并由健康顾问生成随访计划闭环', () => {
  ...
  const followUpModel = fs.readFileSync(path.join(__dirname, '../src/models/FollowUp.js'), 'utf8');
  ...
  assert.match(followUpModel, /'outpatient_reports_audited'/);
  ...
  assert.match(route, /const sourcePlan = isRequiredOutpatientDocument[\s\S]*HealthPlan\.findById/);
  assert.match(route, /if \(isRequiredOutpatientDocument && sourcePlan\)/);
  ...
  assert.match(route, /autoAuditPending[\s\S]*syncOutpatientReportAuditCompletion/);
  assert.match(route, /工作台读取时都按[\s\S]*syncOutpatientReportAuditCompletion/);
