...
(async () => {
  ...
  const revise = buildConfirmation(await users.findOne({ _id: user._id }), [{ path, value: '08:30' }], { _id: new m.Types.ObjectId() });
  assert.equal((await users.updateOne(revise.filter, revise.update)).modifiedCount, 1);
  const reviewed = await users.findOne({ _id: user._id });
  assert.equal(reviewed.lifestyle_data.breakfastTime, '08:30');
  assert.equal(reviewed.archiveVersionHistory, undefined);
  assert.equal(reviewed.archiveConfirmLog.at(-1).items[0].mode, 'initial_revision');
  await users.updateOne({ _id: user._id }, { $set: { archiveDraft: { ...user.archiveDraft, responseId: new m.Types.ObjectId() } } });
  ...
  console.log('PASS first baseline and same-response revision persisted, zero change entries; concurrent manual edit rejects stale confirmation');
