...
function render(stage,own=true){
  ...
  const ctx={module:{exports:{}},require:name=>name==='react'?fakeReact:name==='./CareFlowReviewEvidence'?()=>null:name==='./CareFlowReportUploads'?()=>null:name==='./CareFlowHandoff'?handoff():name==='./CareFlowExaminations'?examinations():name==='../api'?{careFlowAPI:{}}:name.includes('/shared/')?require(path.join(__dirname,'../../shared',path.basename(name))):{}};
  vm.runInNewContext(require('esbuild').transformSync(fs.readFileSync(path.join(__dirname,'../../staff/src/components/CareFlowCard.jsx'),'utf8'),{loader:'jsx',format:'cjs'}).code,ctx);
