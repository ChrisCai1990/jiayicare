...
async function generateHealthSummarySections(user, { scope = 'all', existingSections = null } = {}) {
  ...
  const roleIntro = scope === 'nutrition'
    ? '你是一位经验丰富的营养师，请根据以下会员完整健康档案，仅针对「生活方式评估」维度生成结构化分析。'
    : scope === 'doctor'
      ? '你是一位经验丰富的健康顾问，请根据以下会员完整健康档案，仅针对医疗5个维度（优先医疗问题/肿瘤风险/心脑血管风险/慢病管理/体检完整性）生成结构化分析。'
      : '你是一位经验丰富的健康顾问，请根据以下会员完整健康档案生成结构化综合健康分析报告。';
  ...
  const prompt = `${roleIntro}问题分析必须身心结合，不能只谈躯体指标而忽略心理健康量表数据，反之亦然——如果心理评估分数偏高但躯体指标正常，仍需在情绪维度和风险清单中明确指出；如果慢病/躯体症状可能与情绪压力互为因果，也需在分析中点明关联。建议内容如涉及需要医生跟进，一律使用"健康顾问"这一称呼，不要用"主治医生"（本平台提供的是健康顾问服务）。
