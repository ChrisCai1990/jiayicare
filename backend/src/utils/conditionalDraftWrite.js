...
async function saveConditionalDrafts(HealthPlan, plan, snapshot, decisions) {
  const result = await HealthPlan.updateOne({ _id: plan._id, patientId: plan.patientId,
    status: plan.status, content: snapshot }, { $set: { 'content.workflowModuleDecisions': decisions } });
