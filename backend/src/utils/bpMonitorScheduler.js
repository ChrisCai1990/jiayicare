...
// 会员一旦打卡，healthRecords.js 会自动把对应随访标记为 completed，
  ...
  for (const patientId of patientIds) {
    try {
      ...
      await Message.create({
        ...
        sender: 'AI健康规划师',
        title,
        content: '您的健康方案要求今日监测血压，目前还没有看到您的打卡记录。请尽快测量并录入，如结果异常AI会第一时间提醒您的健康顾问关注。',
        unread: true,
      });
      remindedCount++;
    } catch (e) {
      console.error('[bp-monitor] 会员 ' + patientId + ' 提醒发送失败', e.message);
    }
  }
  if (remindedCount > 0) console.log(`[bp-monitor] 已为 ${remindedCount} 位会员发送血压监测提醒`);
}
