...
function buildAnnualRenewalTodos(plans, periods, staff, now = new Date()) {
  ...
  return plans.flatMap(plan => {
    ...
    return [{ id: `annual_renewal_${plan._id}`, type: 'annual_renewal_confirmation', label: issue.code === 'correction_apply' ? '续约更正·应用受阻' : issue.code === 'correction_review' ? '续约更正·顾问审核' : issue.code === 'correction_rejected' ? '续约更正·已退回' : period ? '年度任务同步异常·待处理' : '下一年度续约凭据待核对', priority: 2,
