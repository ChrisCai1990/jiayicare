...
// 一套需要医护单独手动录入的字段，绝大多数会员几乎为空（如金娟名下会员只有 tc 一个值、labHistory 0 条），
// 导致 AI 拿不到真实数值只能脑补，"提取数据没一次对的"、每次重生成结果差异巨大。
// 会员真实的体检数值全在专项筛查报告的 reportItems 里（上百条带 value）。本模块把这些数值按同一套
...
const REPORT_KEY_MAP = {
  ...
  // 裸"胆固醇"会误命中"高密度脂蛋白胆固醇"(HDL)/"低密度脂蛋白胆固醇"(LDL)——
  // 金娟2026报告TC误取成HDL的1.73(正确应4.42)。排除脂蛋白类，只认真正的总胆固醇。（2026-07-10）
