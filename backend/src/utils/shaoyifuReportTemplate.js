...
const PAGE_RULES = {
  4: '一般项目每行单独输出data。眼科、耳鼻喉科、牙科、妇科必须作为检查项目提取，每个科室只输出一条imaging，科内全部明细按“项目：原文结果”逐行写入findings，严禁把科内明细拆成lab或data检验/数值项目。体重、体重指数、脉搏、跌倒评分不得遗漏；眼科、耳鼻喉科必须逐项核对左右栏及续页，不得漏项；不得读取小结页内容。',
