...
function reviewQueueFilter(role) {
  if (role === 'superadmin') return { status: { $in: ['pending', 'nutrition_review', 'professional_review', 'doctor_review', 'rejected'] } };
  if (role === 'familyDoctor') return { $or: [{ status: 'doctor_review' }, { status: 'rejected', primaryReviewRole: role }] };
  const primaryFilter = role === 'nutritionist' ? { $or: [{ primaryReviewRole: role }, { primaryReviewRole: { $exists: false } }] } : { primaryReviewRole: role };
  return { ...primaryFilter, status: { $in: role === 'nutritionist' ? ['pending', 'nutrition_review', 'rejected'] : ['professional_review', 'rejected'] } };
}
