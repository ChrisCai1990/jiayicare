...
// 对所有已确认方案、本月尚未跑过回顾的会员执行一次月度AI随访回顾
// 每个会员只按其最新确认的方案跑一次（同一会员可能有多个方案类型，取最近确认的一份即可，避免重复生成随访建议）
async function scanAndRunMonthlyReview() {
  ...
  for (const plan of plans) {
    ...
    try {
      ...
      console.error('[monthly-followup-review] 会员 ' + patientKey + ' 回顾失败', e.message);
    } finally {
  ...
  if (count > 0) console.log(`[monthly-followup-review] 已为 ${count} 位会员生成月度随访回顾`);
}
