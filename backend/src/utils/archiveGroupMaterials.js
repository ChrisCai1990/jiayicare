...
const {fileMime,checkedDate,same} = require('./serviceGroupRules');
const fail=(message,status=400)=>{throw Object.assign(new Error(message),{status});};
module.exports = async function archiveGroupMaterials({g,p,staff,ids,purpose,title,date,category}) {
    const results=[];
    for(const id of ids){
      ...
      try {
        ...
        if(receipt){
          ...
          receipt=await Receipt.findOneAndUpdate({_id:receipt._id,state:{$in:['failed','queued']}},{$set:{state:'processing'}},{new:true});
          if(!receipt) fail('该原件正在处理，请勿重复提交；长时间未完成请联系管理员核对',409);
        }else{
          receipt=await Receipt.create({groupId:g._id,messageId:id,patientId:p._id,purpose,title,date,documentCategory:category,staffId:staff._id});
        }
        owned=true;
        ...
        if(!result){
          ...
          try {
            result=await Model.create(purpose==='report'?{
              ...
              sourceSha256:digest,sourceServiceGroup:g._id,sourceGroupMessageId:m.messageId,uploadedBy:staff._id,uploadedByRole:staff.role,
              aiStatus:'none',audit_status:'unaudited',
            }:{patientId:p._id,staffId:staff._id,type:'group_service',title:'日常检测原图 · '+title,date:checkedDate(date),
              content:'已核对归属的日常检测原图；未自动提取数值，不代表指标已审核。',
