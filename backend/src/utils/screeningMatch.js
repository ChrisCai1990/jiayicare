...
function norm(s) {
  ...
  // 去括号，但括号内是较短的字母/数字/符号代码（如「脂蛋白(a)」的"a"、「维生素D(25-羟)」的"25-羟"、
  // 「细胞(CD3+)」的"CD3+"）时，只去括号本身、保留内容——这类代码往往是区分不同检验项目的关键标识，
  // 整体删掉会导致退化成过短的通用词（如"脂蛋白""细胞"），进而被其他所有含相同通用词的项目误配。
  // 2026-07-03：原规则只保留纯1-3位字母数字，"CD3+"因为带"+"号仍被整体删除，导致"细胞(CD3+)"退化成
  // "细胞"这个2字通用别名，误配所有含"细胞"字样的项目（白细胞/淋巴细胞/细胞角蛋白...）。
  // 放宽为允许字母数字加常见符号(+-/)，长度放宽到6位，覆盖CD3+/CD4+/25-羟等免疫学、生化常见简写。
  // 括号内是较长中文说明文字（如「葡萄糖(空腹)」的"空腹"）时仍整体删除，不影响原有效果。
  t = t.replace(/[（(【\[]([^）)】\]]*?)[）)】\]]/g, (m, inner) => /^[a-z0-9+\-/]{1,6}$/i.test(inner) ? inner : '');
