...
{"title":"本次随访主题（10字内）","content":"随访要点摘要（100-200字，包含会员近期状态/主诉、医生实际给出的回应或建议、会员反馈情况）","result":"结论性评估（30-50字，基于聊天记录总结，供医生审核确认）","nextDate":"YYYY-MM-DD 或 null"}`
    : `你是${label}的随访记录助手。以下是${label}与会员${user.name}在${days === 1 ? '当日' : days + '天内'}的聊天记录，请提炼成一条"${recordLabel}"服务记录，不要逐句复述聊天内容，要总结随访的核心信息。若聊天记录与健康管理无实质关联，content中如实说明"本次沟通无实质随访内容"。
