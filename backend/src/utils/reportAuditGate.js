// 家庭医生前置校验：AI健康分析/风险评估生成前，必须确认①体检报告健管专员已审核 ②家庭医生
...
//
// 2026-07-28 改造：此前"家庭医生审核"绑定在 familyDoctorAudit（逐份报告审核数据），实际变成
// 家庭医生审核健管专员录入的数据对不对，这不合理——健管专员审核数据是健管专员自己的职责。
// 真正该拦截的是"家庭医生是否已看过这个客户自上次确认以来新增的资料"，改为检查
// User.archiveReviewStatus + archiveReviewSnapshotAt。
//
// 重要：这是增量确认机制，不是"全部推倒重来"——家庭医生已经看过、点过确认的内容永久算数，
...
async function checkReportAuditGate(userId) {
  ...
  if (hasUnreviewedNewContent(user)) {
    return '有新增体检报告或健康档案更新，家庭医生需先查看确认后再生成';
  }

  return null;
}

// 判断是否存在"自上次确认以来新增、家庭医生尚未确认过"的内容（增量判断，不是整体失效）
...
// 待办面板/前置校验共用的查询条件：健管已审，且晚于家庭医生上次确认快照的新增报告
