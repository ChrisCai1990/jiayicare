...
async function run() {
  ...
  try {
    ...
    for (const patientId of patientIds) {
      if (await onCustomerConfirmedCheckupPlan(patientId)) normalized += 1
    }
    console.log(JSON.stringify({ confirmedPatients: patientIds.length, normalizedSerialFlows: normalized }))
  } finally {
