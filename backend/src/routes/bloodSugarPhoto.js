...
const { PROMPT, parseResult, issueDraft } = require('../utils/bloodSugarPhoto');
...
router.post('/recognize-blood-sugar', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, data: { ...values, token: issueDraft(key, hash, values),
      message: '请核对原图和血糖值，并选择测量状态与时间；空白表示无法确定，请手工补填。' } });
  } catch {
