...
【规则A：绝对禁止提取会员基本信息字段及行政字段】
以下字段是会员个人信息或行政信息，一律跳过，不生成任何条目：
...
⑤ 会员基本信息栏（姓名/年龄/性别/电话/单位/体检日期等表头行）
...
【规则H：diagnosis/conclusion 严禁把中文原文自行替换/翻译成英文】
部分检查（如宫颈液基细胞学/TCT病理）的诊断结论，报告原文印刷的其实是中文（如"未见上皮内
病变或恶性病变"），但你可能凭自己知道的TBS分类法英文术语，把它替换/联想成了英文短语
（如"Negative for intraepithelial lesion or malignancy"）填进 diagnosis——这是严重的幻觉
错误，属于规则零"禁止推断和补全"的违反：报告上写的是什么文字，就原样提取什么文字，绝对
不能用你自己知道的专业术语（无论中译英还是英译中）替换报告原文。
✓ 正确：报告印刷"（宫颈液基）未见上皮内病变或恶性病变" → diagnosis 原样填
  "未见上皮内病变或恶性病变"
✗ 错误：diagnosis 填 "Negative for intraepithelial lesion or malignancy"（报告原文明明是
  中文，却擅自替换成了英文专业术语）
→ 若报告原文本身确实印刷的是英文（少数境外机构报告），才允许翻译成中文标准表述后填入；
  但绝大多数国内体检报告的诊断结论都是中文原文，遇到中文原文时严禁擅自替换成英文。
→ name（项目名称）、value（数值）、unit（单位）、findings 中的英文缩写指标名（如 DOB、
  CRP、IgG 等专业代号）不受此规则约束，仍按报告原文提取。
...
【3. 内科/外科/全科/耳鼻喉/牙科或口腔科/妇科/听力/视力/眼压/眼科及其他一般体格检查项目】
→ 【重要】这类项目是体格检查，不是检验项目。itemType="imaging"；报告中每个印刷明细项目都单独输出一条，严禁把整个科室合并成一条大段文字。
→ name = 明细项目名称，与报告原文完全一致；sourceSection = 科室名称。
→ findings = 该明细项目同行的完整结果原文，包括正常结果，不得省略、压缩、摘要。
→ 眼科逐行核对左右栏，原图出现“前房清”“周边前房深度右”“周边前房深度左”时必须分别输出三条，不得遗漏。
  ✓ 正确示例（眼科）：分别输出 name="前房清" findings="是"、name="周边前房深度右" findings="≥1"、name="周边前房深度左" findings="≥1"
  ✓ 正确示例（耳鼻喉）：分别输出 name="耳部检查"、name="鼻部检查"、name="咽部检查"，各自 findings 只写同行结果
  ✓ 正确示例（全科）：家族史、疾病史、手术史、心脏、胸部、腹部等分别输出独立条目
  ✓ 正确示例（妇科/口腔科）：外阴、阴道、宫颈、子宫、附件或龋齿、牙周、牙龈等原图项目分别输出
  ✗ 禁止：只写"未见异常" / 只写有异常的项目 / 合并压缩成一句话
  ✗ 禁止：把体格检查明细误标成 lab；明细仍须独立输出，但 itemType 必须是 imaging
...
→ 【重要】diagnosis 和 conclusion 字段一律留空字符串。数值型检验项只提取数值与参考范围，异常与否由 status 表达，绝对禁止给检验项编写"未见异常""肌酐水平升高提示肾功能异常"之类的诊断意见文字——诊断意见只属于影像/检查类(imaging)项目。
...
→ 【orderName 必填，填写该子项所属的检验单大标题】：
  ...
  尿微量白蛋白类：尿微量白蛋白/尿肌酐(U-Cr)/白蛋白肌酐比值(ACR)/尿N-乙酰-β-D-葡萄糖苷酶(NAG)/尿转铁蛋白/尿免疫球蛋白 等尿液标本项目 → "尿微量白蛋白"或"尿肾功能"（按报告实际分组）
...
【6. 尿常规 / 粪便常规】
→ 按检验单整体提取，不拆成检验子项；每张尿常规、粪便常规各输出一条。
→ itemType="lab"
→ name = 检查名称（"尿常规"/"粪便常规"）
...
→ value/unit/referenceRange固定留空，不提取各子项参考范围
→ diagnosis/conclusion留空；status按全部子项综合填写

【7. 碳13/碳14呼气试验】
该检查既有具体测定数值（DOB值）又有阳性/阴性结论文字，跟心电图/肺功能一样需提取两类条目，不能只写一条imaging把数值丢在文字里：

① 测量数值（itemType="lab"，一条）：
   → name="碳13尿素呼气试验"或"碳14尿素呼气试验"（严格区分碳13/碳14，不得改名）
   → value=DOB测定值；unit=单位（通常‰或DOB）；referenceRange=报告印刷的阳性判断阈值/参考范围（如"<4.0为阴性"，无则留空）；status=normal/abnormal
   → orderName="幽门螺杆菌检测"
   → diagnosis/conclusion 留空字符串（数值型条目不写结论，规则同【5】）

② 检查结论（itemType="imaging"，一条）：
   → name=同①的报告原文名称
   → findings = 检测结果完整原文（含①已提取的数值，允许重复出现）
   → diagnosis = 结论原文（如"幽门螺杆菌阳性"/"阴性"及完整原文）
   → conclusion = 同 diagnosis 原文
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    // contents：多图上传时的 base64 数组（同一份报告的多页，如"结论页"+"数据页"）；content 仍保留
    // 兼容单图上传的旧调用方。两者互斥，contents 存在时优先。
    const contentList = Array.isArray(contents) && contents.length ? contents : (content ? [content] : []);
    ...
    const report = await MedicalReport.create({
      ...
      pages:              pages              || contentList.length || 1,
      fileSize:           fileSize           || '',
      fileUrl:            fileUrl            || '',
      fileUrls:           fileUrls,
      ossKey:             ossKey,
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    // 删除 OSS 文件（多图上传时逐个清理，避免只删 ossKeys[0] 留下孤儿文件）
    const keysToDelete = report.ossKeys && report.ossKeys.length ? report.ossKeys : (report.ossKey ? [report.ossKey] : []);
    for (const k of keysToDelete) await deleteFile(k);
