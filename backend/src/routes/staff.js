...
14. 人体成分分析（InBody/BCA-2A等体成分测量仪报告）
    ...
    → 这类报告页面顶部常同时印有"健康评分"（一个0-100的综合评分数字）和"体重"（kg数值），
      两者版面位置接近、都是不带单位说明的纯数字，极易看错行/张冠李戴——这两项单独按规则1
      （一般检查）各自提取一条 itemType="data"，不要混入"人体成分分析"这条 imaging 记录里。
      必须严格按各自的标签文字取值：name="健康评分"只能取"健康评分"这个标签紧跟着的数字；
      name="体重"必须取报告下方"基本评估"表格里"体重"那一行、带kg单位的公斤数值。
