...
5. 所有血液检查（肝肾功能/血糖/血脂/肿瘤标志物/尿微量白蛋白/尿肌酐/抗核抗体谱/血常规等免疫指标）
   ...
   → name/value/unit/referenceRange/status 逐项填写；status只抄报告单该行印刷的箭头、H/L、阴阳性或明确正常/异常标记，报告没有标记就填"unknown"，禁止根据数值和参考范围自行判断
...
6. 尿常规 / 粪便常规
   ...
   → itemType="lab"，name=该行检查项目原名，value/unit/referenceRange/status严格取该行内容，orderName=报告实际印刷的检验单标题；无印刷状态标记时status="unknown"，不得自行计算
...
8. 超声（肝脏/胆囊/胰腺/脾脏/双肾输尿管膀胱/前列腺/甲状腺/颈动脉/心脏超声/乳腺/子宫附件或阴道等）
   → 只输出当前图片中能够同时明确读到检查名称和对应正文的项目；看不清、正文缺失或归属不确定时不要输出，严禁根据组合标题、常见套餐或医学常识补造器官和“未见异常”结果
   → 原报告按器官分段时可按原段落拆分；原报告是组合检查且无法可靠分段时保留原组合名称和原文，不强制展开成多个器官
   → name、findings、diagnosis 必须逐字来自当前图片；findings/diagnosis 只能属于同一个明确器官或原组合检查，禁止顺移、改写或复制到其他器官
   → conclusion = 同 diagnosis
   → 跳过"温馨提示""健康建议"类科普说明文字（如"结石多与饮水少有关，建议..."），这类不是检查所见，不得提取为 findings

   → “上述未见异常”“余未见异常”等无法唯一归属的文字保留在原组合项目中，不复制到多个器官
