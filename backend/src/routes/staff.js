...
// 保证从工作台/随访管理点进某个患者详情页，看到的随访记录范围是一致的。
...
router.post('/patients/:id/chat-followup/ai-draft', staffAuth, checkPermission('service_records', 'create'), async (req, res) => {
  try {
    ...
    if (result.status === 'skip') return res.status(result.message === '患者不存在' ? 404 : 400).json({ success: false, message: result.message });
...
router.get('/ai-todos', staffAuth, async (req, res) => {
  try {
    ...
    // ── 健管专员/家庭医师/营养师：AI从聊天记录提炼的随访草稿待审核（ServiceRecord.aiStatus=pending）──
...
规则K（2026-07-21新增）：items 数组的输出顺序必须跟随报告原文里各检验单/检查项目实际印刷的先后顺序（从上到下、从前到后逐页读取），不能按itemType(lab/imaging/data)、器官系统、或任何AI自己认为的"逻辑分类"重新排序、打乱穿插。举例：报告依次印刷"一般检查→尿常规→血常规→肝功能→胸部CT"，输出的items就必须严格按这个顺序排列，不能先把所有lab类整理在一起再把imaging类放最后，也不能把内容相关的项目（如所有肿瘤标志物）汇总挪到一起。此规则只约束"输出顺序"，不影响上面各条按检查类型制定的拆分/合并规则本身——例如超声仍需按规则8拆成多条独立记录、胃肠镜仍需按规则10合并成一条，只是这些拆/合出来的条目在数组中出现的位置，要放在该检查项目在原文中实际出现的位置上，不要因为拆分或合并而打乱了整体顺序。
...
3. 内外科 / 耳鼻喉 / 视力检查 / 眼压检查 / 眼科检查
   → 体格检查项目，非检验项目，itemType="imaging"，报告原文写了几科就提取几条，
