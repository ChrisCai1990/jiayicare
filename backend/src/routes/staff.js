...
const MonthlyServiceReview = require('../models/MonthlyServiceReview');
...
async function getMedicationProxyDefaults(patientId, selectedMedicationId = '') {
  const RecurringSupplyPlan = require('../models/RecurringSupplyPlan');
  const medications = await Medication.find({ user: patientId, stopped: { $ne: true }, active: { $ne: false }, aiStatus: { $ne: 'pending' } })
    .sort({ 'supplyReminder.updatedAt': -1, updatedAt: -1 }).lean();
  const linkedFollowUp = await FollowUp.findOne({
    patientId,
    sourceType: 'supply_reminder',
    status: { $in: ['planned', 'in_progress', 'missed'] },
    $or: [{ theme: /我方代配|代配药|代取药/ }, { tags: { $in: ['我方代配', '代配药'] } }],
  }).sort({ date: 1, updatedAt: -1 }).lean();
  let medication = selectedMedicationId
    ? medications.find(item => String(item._id) === String(selectedMedicationId))
    : null;
  if (!medication) medication = linkedFollowUp?.sourceId
    ? await Medication.findOne({ _id: linkedFollowUp.sourceId, user: patientId, stopped: { $ne: true }, active: { $ne: false } }).lean()
    : null;
  if (!medication) medication = await Medication.findOne({ user: patientId, stopped: { $ne: true }, active: { $ne: false }, 'supplyReminder.enabled': true, 'supplyReminder.mode': 'proxy' }).sort({ 'supplyReminder.updatedAt': -1, updatedAt: -1 }).lean();
  if (!medication) medication = medications[0] || null;
  ...
  if (!medication) return { medicationOptions };
  ...
  return {
    ...
    sourceLabel: medicationFollowUp ? '已关联的代配药任务' : (plan ? '定期配药计划' : '药物档案'),
    sourceFollowUpId: medicationFollowUp?._id || medication.supplyReminder?.followUpTaskId || null,
...
router.get('/patients/:id/medication-proxy/defaults', staffAuth, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: await getMedicationProxyDefaults(req.params.id, req.query.medicationId) });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
...
router.post('/patients/:id/medical-proxy/start', staffAuth, async (req, res) => {
  ...
  try {
    ...
    const required = medicalEscort ? ['escortCategory', 'escortDate', 'escortTime', 'hospital', 'escortGoal'] : medicationProxy ? ['preferredDateStart', 'medicationName', 'medicationBrand', 'medicationSpecification', 'medicationQuantity'] : appointmentOnly ? ['hospital', 'department', 'preferredDateStart', 'preferredDateEnd'] : ['hospital', 'department', 'expert', 'proxyGoal', 'communicationContent'];
    if (required.some(key => !String(req.body[key] || '').trim())) return res.status(400).json({ success: false, message: medicalEscort ? '请完整填写服务日期时间、医院和具体服务事项' : medicationProxy ? '请选择服务日期；药物名称、品牌、规格和数量应从关联代配药任务自动获取' : appointmentOnly ? '请完整填写医院、科室和期望日期区间' : '请完整填写医院、科室、专家、代诊目标和交流内容' });
    if (medicalEscort) {
...
router.post('/products/:id/push', staffAuth, checkPermission('products', 'send'), async (req, res) => {
  ...
  const cleanPerformers = Array.isArray(servicePerformers)
    ...
    : [];
  const productItem = {
    productId: product._id.toString(), name: product.name,
    price: product.originalPrice ?? 0, category: product.category || '', icon: '🛍',
    images: product.images || [], servicePrices: product.servicePrices || [],
  };
  const records = patientIds.map(pid => ({
    staffId: req.staff._id, patientId: pid,
    type: 'product',
...
router.patch('/referrals/:id', staffAuth, async (req, res) => {
  ...
  const referral = await Referral.findOne({ _id: req.params.id, toStaffId: req.staff._id });
  if (!referral) return res.status(404).json({ success: false, message: '转介记录不存在或无权操作' });
  const originalFeedbackDigest = require('../utils/referralAssessmentWorkflow').referralAssessmentSource(referral).digest;
  const wasCompleted = referral.status === 'completed';
  ...
  if (status === 'completed' && (!wasCompleted || require('../utils/referralAssessmentWorkflow').referralAssessmentSource(referral).digest !== originalFeedbackDigest)) referral.feedbackVersion = (referral.feedbackVersion || 0) + 1;
  await referral.save();
...
router.post('/patients/:id/professional-health-assessments', staffAuth, async (req, res) => {
  ...
  const row = await ProfessionalHealthAssessment.create({
    ...
    followUpAutomation: ['familyDoctor', 'superadmin'].includes(req.staff.role) ? require('../utils/assessmentFollowUpAutomation').initialFollowUpAutomation(purpose) : { status: 'idle' },
...
router.patch('/professional-health-assessments/:assessmentId/review', staffAuth, async (req, res) => {
  ...
  if (action === 'take_over_followups') {
    if (!['familyDoctor', 'superadmin'].includes(req.staff.role)) return res.status(403).json({ success: false, message: '仅健康顾问可接管随访草稿' });
    if (row.status !== 'advisor_review' || row.followUpAutomation?.status !== 'failed' || req.body.revision !== row.__v) return res.status(409).json({ success: false, message: '评估已更新，请刷新后查看' });
    row = await ProfessionalHealthAssessment.findOneAndUpdate({ _id: row._id, status: 'advisor_review', __v: row.__v, 'followUpAutomation.status': 'failed' }, {
      $set: { 'followUpAutomation.status': 'ready', 'followUpAutomation.message': '健康顾问已接管，请人工核对并补充必要随访；不需要新增时可保留空列表后终审。' },
      $inc: { __v: 1 }, $push: { auditLog: { action, at: new Date(), by: req.staff._id, role: req.staff.role } },
    }, { new: true });
    if (!row) return res.status(409).json({ success: false, message: '评估已更新，请刷新' });
  ...
  if (action === 'approve_advisor') {
    ...
    if (row.status === 'advisor_review') {
      ...
      if (!(await require('../utils/referralAssessmentWorkflow').isAssessmentSourceCurrent(row))) return res.status(409).json({ success: false, message: '来源转介反馈已修订，请改为审核最新评估' });
      if (['queued', 'running', 'failed'].includes(row.followUpAutomation?.status)) return res.status(409).json({ success: false, message: row.followUpAutomation?.message || '随访草稿尚未完成，请刷新或重试生成后终审' });
      let drafts;
  ...
  if (action === 'reject') {
    ...
    if (!['draft', 'professional_review', 'rejected'].includes(row.status)) return res.status(409).json({ success: false, message: '评估已提交健康顾问，无需重复提交' });
    ...
    if (!row.followUpDraftGeneratedAt && !row.followUpDrafts?.length && ['idle', undefined].includes(row.followUpAutomation?.status)) row.followUpAutomation = require('../utils/assessmentFollowUpAutomation').initialFollowUpAutomation(row.purpose);
  }
  ...
  else await require('../utils/referralAssessmentWorkflow').ensureAdvisorReviewTask(row);
...
router.post('/professional-health-assessments/:assessmentId/ai-followup-draft', staffAuth, async (req, res) => {
  ...
  try {
    await require('../utils/referralAssessmentWorkflow').ensureAdvisorReviewTask(row);
    const updated = await require('../utils/assessmentFollowUpAutomation').runAssessmentDraft(row._id, { revision: req.body.revision, allowRevision: req.body.allowRevision === true });
    if (!updated) return res.status(409).json({ success: false, message: '评估已更新，请刷新' });
    res.json({ success: true, data: { followUpDrafts: updated.followUpDrafts || [], generatedAt: updated.followUpDraftGeneratedAt, revision: updated.__v, automation: updated.followUpAutomation } });
  } catch (error) { res.status(error.statusCode || 500).json({ success: false, message: error.statusCode ? error.message : '生成未完成，请刷新后重试' }); }
});
...
function monthlyReviewMonthRange(month) {
  const [year, number] = month.split('-').map(Number);
  return { $gte: new Date(Date.UTC(year, number - 1, 1) - 8 * 3600000), $lt: new Date(Date.UTC(year, number, 1) - 8 * 3600000) };
}
async function monthlyReviewFacts(patientId, month) {
  const range = monthlyReviewMonthRange(month);
  const [followUps, completed, serviceRecords] = await Promise.all([
    FollowUp.countDocuments({ patientId, date: range, status: { $ne: 'cancelled' } }),
    FollowUp.countDocuments({ patientId, date: range, status: 'completed' }),
    ServiceRecord.countDocuments({ patientId, date: range, aiStatus: { $ne: 'pending' } }),
  ]);
  return { followUps, completed, serviceRecords, generatedAt: new Date(), note: '仅汇总系统已有记录数量；完成数不代表服务目标达成，缺失资料须人工核实。' };
}

router.get('/patients/:id/monthly-service-reviews', staffAuth, async (req, res) => {
  try {
    const visible = await getVisiblePlanPatientIds(req.staff);
    const patientScope = !visible || visible.some(id => String(id) === String(req.params.id));
    if (!mongoose.isValidObjectId(req.params.id) || (!patientScope && !(await MonthlyServiceReview.exists({ patientId: req.params.id, 'actions.assigneeId': req.staff._id })))) return res.status(403).json({ success: false, message: '无权查看该客户复盘' });
    if (!require('../utils/monthlyReviewRollout').enabledForPatient(req.params.id)) return res.json({ success: true, enabled: false, data: { plans: [], reviews: [], canOrganize: false } });
    const allPlans = await AnnualPlan.find({ patientId: req.params.id, confirmedAt: { $ne: null } }).select('_id year servicePlanCode templateName confirmedAt').sort({ confirmedAt: -1 }).lean();
    const seenYears = new Set();
    let plans = allPlans.filter(plan => {
      if (seenYears.has(plan.year)) return false;
      seenYears.add(plan.year); return true;
    });
    const rows = await MonthlyServiceReview.find({ patientId: req.params.id, ...(patientScope ? {} : { 'actions.assigneeId': req.staff._id }) }).populate('confirmedBy contributions.by actions.assigneeId actions.completedBy corrections.by', 'name role').sort({ month: -1 }).lean();
    if (!patientScope) plans = plans.filter(plan => rows.some(row => String(row.annualPlanId) === String(plan._id)));
    res.json({ success: true, enabled: true, data: { plans, reviews: rows, currentMonth: require('../utils/monthlyServiceReview').shanghaiMonth(), canOrganize: await monthlyReviewOrganizerAccess(req.staff, req.params.id) } });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});

router.post('/patients/:id/monthly-service-reviews', staffAuth, async (req, res) => {
  try {
...
router.patch('/monthly-service-reviews/:reviewId', staffAuth, async (req, res) => {
  try {
    ...
    if (Number(req.body.revision) !== row.__v) return res.status(409).json({ success: false, message: '复盘已更新，请刷新后再保存' });
    const sections = {};
    for (const key of monthlyReviewFields) {
      if (typeof req.body.sections?.[key] !== 'string' || req.body.sections[key].length > 5000) return res.status(400).json({ success: false, message: `请核对${key}内容` });
      sections[key] = req.body.sections[key].trim();
    }
    if (!Array.isArray(req.body.actions) || req.body.actions.length > 30) return res.status(400).json({ success: false, message: '行动项最多30条' });
    const actions = req.body.actions.map(action => ({
      title: String(action.title || '').trim(), assigneeId: action.assigneeId?._id || action.assigneeId,
      dueAt: action.dueAt, status: 'pending',
      ...(action._id && row.actions.some(item => String(item._id) === String(action._id)) ? { _id: action._id } : {}),
    }));
    if (actions.some(action => !action.title || action.title.length > 200 || !mongoose.isValidObjectId(action.assigneeId) || !action.dueAt || Number.isNaN(Date.parse(action.dueAt)))) return res.status(400).json({ success: false, message: '每项行动须填写事项、负责人及期限' });
    const staffIds = [...new Set(actions.map(item => String(item.assigneeId)))];
    if (staffIds.length && await Admin.countDocuments({ _id: { $in: staffIds }, staffStatus: 'active' }) !== staffIds.length) return res.status(400).json({ success: false, message: '行动负责人不存在或已停用' });
    const updated = await MonthlyServiceReview.findOneAndUpdate({ _id: row._id, __v: row.__v, status: 'draft' }, { $set: { sections, actions }, $inc: { __v: 1 }, $push: { auditLog: { action: 'edited', by: req.staff._id, at: new Date() } } }, { new: true, runValidators: true });
    if (!updated) return res.status(409).json({ success: false, message: '复盘已更新，请刷新' });
    res.json({ success: true, data: updated });
  } catch (error) { res.status(error.name === 'ValidationError' || error.name === 'CastError' ? 400 : 500).json({ success: false, message: error.message }); }
});

router.post('/monthly-service-reviews/:reviewId/contributions', staffAuth, async (req, res) => {
  try {
    ...
    if (!monthlyReviewFields.includes(section) || !content || content.length > 2000) return res.status(400).json({ success: false, message: '请选择栏目并填写2000字以内的内容' });
    const updated = await MonthlyServiceReview.findOneAndUpdate({ _id: row._id, status: 'draft' }, { $push: { contributions: { section, content, by: req.staff._id, at: new Date() }, auditLog: { action: 'contributed', by: req.staff._id, at: new Date() } }, $inc: { __v: 1 } }, { new: true });
    if (!updated) return res.status(409).json({ success: false, message: '复盘已确认，请刷新' });
    res.json({ success: true, data: updated });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});

router.post('/monthly-service-reviews/:reviewId/confirm', staffAuth, async (req, res) => {
  try {
...
router.post('/monthly-service-reviews/:reviewId/corrections', staffAuth, async (req, res) => {
  try {
    const row = await MonthlyServiceReview.findById(req.params.reviewId).select('patientId status').lean();
    if (!row) return res.status(404).json({ success: false, message: '月度复盘不存在' });
    ...
    const updated = await MonthlyServiceReview.findOneAndUpdate({ _id: row._id, status: 'confirmed' }, { $push: { corrections: { content, by: req.staff._id, at: new Date() }, auditLog: { action: 'corrected', by: req.staff._id, at: new Date() } }, $inc: { __v: 1 } }, { new: true });
    if (!updated) return res.status(409).json({ success: false, message: '复盘状态已变化，请刷新' });
    res.json({ success: true, data: updated });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});

router.post('/monthly-service-reviews/:reviewId/actions/:actionId/complete', staffAuth, async (req, res) => {
  try {
    ...
    if (row.status !== 'confirmed' || action.status !== 'pending') return res.status(409).json({ success: false, message: '行动项状态已变化，请刷新' });
    const updated = await MonthlyServiceReview.findOneAndUpdate({ _id: row._id, status: 'confirmed', actions: { $elemMatch: { _id: action._id, status: 'pending' } } }, { $set: { 'actions.$.status': 'completed', 'actions.$.completedAt': new Date(), 'actions.$.completedBy': req.staff._id }, $inc: { __v: 1 }, $push: { auditLog: { action: 'action_completed', by: req.staff._id, at: new Date() } } }, { new: true });
    if (!updated) return res.status(409).json({ success: false, message: '行动项已处理，请刷新' });
    res.json({ success: true, data: updated });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});

router.get('/monthly-service-reviews/workbench', staffAuth, async (req, res) => {
  try {
    const rollout = require('../utils/monthlyReviewRollout');
    const { dueMonths } = require('../utils/monthlyServiceReview');
    const isPlanner = ['healthPlanner', 'superadmin'].includes(req.staff.role);
    const patientFilter = rollout.patientFilter();
    if (req.staff.role === 'healthPlanner') {
      const ownIds = await User.find({ assignedHealthPlanner: req.staff._id, isDeleted: { $ne: true } }).distinct('_id');
      patientFilter.patientId = { $in: ownIds.filter(id => rollout.enabledForPatient(id)) };
    ...
    }).map(month => ({ annualPlanId: plan._id, patientId: plan.patientId?._id, patientName: plan.patientId?.name, month, reviewId: byPatientMonth.get(`${plan.patientId?._id}:${month}`)?._id || null })));
    const assignedReviews = await MonthlyServiceReview.find({ 'actions.assigneeId': req.staff._id, status: 'confirmed', ...rollout.patientFilter() }).select('patientId annualPlanId month actions').lean();
    const actions = assignedReviews.flatMap(review => review.actions.filter(action => action.status === 'pending' && String(action.assigneeId) === String(req.staff._id)).map(action => ({ reviewId: review._id, annualPlanId: review.annualPlanId, patientId: review.patientId, month: review.month, actionId: action._id, title: action.title, dueAt: action.dueAt })));
    res.json({ success: true, data: { pending, actions } });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});
...
router.post('/orders/:id/medication-draft', staffAuth, async (req, res) => {
  try {
    ...
    const { chat } = require('../utils/ai');
    const raw = await chat([{ role: 'user', content: `只整理明确的代配药信息，不推断。保留单次服用剂量、每日服用次数和配备总量的原始计量单位，例如5mg、1次、4盒；系统将结合规格换算可服用天数。对话缺失时可从持续用药档案补充；冲突或不确定则留空。输出严格JSON：{"brandName":"","chemicalName":"","specification":"","singleDose":"","dailyFrequency":"","totalQuantity":"","institutionType":"hospital或pharmacy或online或空","hospitalName":"","campus":"","pharmacyName":"","platformName":"","paymentMethod":"self_pay或medical_insurance或commercial_insurance或空","department":"","expert":"","expertRequired":false,"needsAdvisor":false,"regularSupply":false,"missingFields":[]}。用药档案：${JSON.stringify(medications).slice(0, 12000)}。订单备注：${String(order.note || '').slice(0, 1000)}。订单对话：${JSON.stringify(messages).slice(0, 18000)}` }], { maxTokens: 900, temperature: 0, jsonMode: true, timeoutMs: 60000 });
    res.json({ success: true, data: JSON.parse(String(raw).match(/\{[\s\S]*\}/)?.[0] || '{}') });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
...
router.patch('/orders/:id/start', staffAuth, async (req, res) => {
  try {
    ...
    if (medicationProxyWorkflow.isMedicationProxyOrder(currentOrder)) {
      ...
      return res.json({ success: true, data: order, task, message: '代配药已进入健康规划师核对环节' });
    }
...
const TODO_REVIEW_ROLE = {
  report_plan_conflict: 'healthManager',
...
async function runReportParse(reportId) {
  const owner = await require('../models/MedicalReport').findById(reportId);
...
async function runReportParseControlled(reportId) {
  ...
  try {
    if (isPdf) {
        ...
        onBatch: async (batchImages, batchIndex) => {
          ...
          const worker = async () => {
            while (cursor < batchImages.length) {
              ...
              const cachedPage = report.parseJob?.firstPassPages?.[pageNum];
              if (cachedPage) { batchResults[i] = cachedPage; continue; }
          ...
          const settled = await Promise.allSettled(Array.from({ length: Math.min(CONCURRENCY, batchImages.length) }, worker));
      ...
      for (const pageNum of bodyCompRetryPages) {
        try {
          ...
          } catch (chartError) { rethrowAiControl(chartError);
    ...
    for (let imageIndex = 0; imageIndex < bufs.length; imageIndex++) {
      const cachedImage = report.parseJob?.imagePages?.[imageIndex + 1];
      if (cachedImage) {
        imageItems.push(...cachedImage.items);
        if (cachedImage.summary) imageSummaries.push(cachedImage.summary);
        if (!imageInstitution) imageInstitution = cachedImage.institution || '';
        if (!imageCheckDate) imageCheckDate = cachedImage.checkDate;
        if (cachedImage.evidence) imagePageEvidence[imageIndex + 1] = cachedImage.evidence;
        imageOkCount++;
        continue;
      }
      try {
        ...
        if (parsedPage?.imageEvidence?.status !== 'image_only' && report.type !== 'body_comp' && (!parsedPage || !Array.isArray(parsedPage.items) || parsedPage.items.length === 0)) {
          for (const degrees of [90, 270, 180]) {
            ...
            } catch (orientationError) { rethrowAiControl(orientationError);
        ...
        if (parsedPage.imageEvidence?.status === 'image_only' || (shouldSkipParsedReportPage(parsedPage) && report.type !== 'body_comp')) {
          ...
          await MedicalReport.findByIdAndUpdate(reportId, { $set: { [`parseJob.imagePages.${imageIndex + 1}`]: { items: [], evidence: imagePageEvidence[imageIndex + 1] } } });
          continue;
        }
        ...
        if (!isBodyCompPage) {
          ...
          } catch (auditError) { rethrowAiControl(auditError);
        ...
        if (!isBodyCompPage) {
          ...
          if (isUpperCombo && beforeUpperCount < 4) {
            ...
            } catch (upperError) { rethrowAiControl(upperError);
        ...
        await MedicalReport.findByIdAndUpdate(reportId, { $set: { [`parseJob.imagePages.${imageIndex + 1}`]: { items: imageItems.filter(item => Number(item.sourcePage) === imageIndex + 1), summary: parsedPage.summary || '', institution: imageInstitution, checkDate: imageCheckDate, evidence: imagePageEvidence[imageIndex + 1] } } });
      } catch (e) { rethrowAiControl(e);
    ...
    if (isAiControlError(e)) {
      await MedicalReport.findByIdAndUpdate(reportId, { $set: { aiStatus: 'failed', 'parseJob.status': 'paused', 'parseJob.message': e.message, 'parseJob.pausedAt': new Date(), 'parseJob.pauseCode': e.code } });
      return;
    }
...
async function runReportPageParse(reportId, pageNum) {
  return withAiContext({ business: 'ocr', reportId: String(reportId), page: pageNum, stage: 'supplement', stopState: {}, deadline: Date.now() + 15 * 60000 }, () => runReportPageParseControlled(reportId, pageNum));
}
...
router.post('/medical-reports/:id/parse-page', staffAuth, async (req, res) => {
  try {
    ...
    const pageClaim = await MedicalReport.findOneAndUpdate({ _id: report._id, aiStatus: { $ne: 'processing' }, 'parseJob.status': { $ne: 'paused' }, $or: [{ 'pageParseStatus.status': { $ne: 'processing' } }, { 'pageParseStatus.startedAt': { $lt: new Date(Date.now() - 20 * 60000) } }] }, { pageParseStatus: { pageNum, status: 'processing', startedAt: new Date(), message: `正在补提第${pageNum}页` } });
    if (!pageClaim) return res.status(409).json({ success: false, message: '报告正在识别、补提或已暂停，请勿重复提交' });
