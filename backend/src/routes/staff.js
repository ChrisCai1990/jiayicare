...
router.get('/ai-todos', staffAuth, async (req, res) => {
  try {
    ...
    if (isSuper || ['healthPlanner', 'familyDoctor'].includes(role)) {
      ...
      const periods = await require('../models/AnnualServicePeriod').find({ annualPlanId: { $in: renewalPlans.map(plan => plan._id) } }).select('annualPlanId activationStatus activationError syncState syncIssue syncStartedAt correction.status correction.reviewNote').lean();
