...
13. 睡眠呼吸监测 / 动态血压监测（24小时动态血压）
    → 【重要】这类报告必须同步按"检验"+"检查"两种方式提取，不是二选一：
      ① 报告里印刷了具体测量数值+单位/参考范围的项（如AHI指数、最长呼吸暂停时长、最长低通气时长、
         最低血氧饱和度、平均血氧饱和度、氧减指数、平均血压、血压负荷值等），逐项用 itemType="lab"
         （name/value/unit/referenceRange）单独一条，不要漏项——数值表格里出现的每一行都要提取，
         不能只挑1-2个"看起来主要"的指标
      ② 【必须做】睡眠监测报告通常在数值表格之后印有整段文字结论，标题常见"医生诊断意见"
         "初筛睡眠监测图诊断""诊断意见""检查提示"之类；动态血压报告则是"提示存在xx型血压"这类
         整体判断句。只要报告里出现了这类文字段落（不管标题叫什么），必须原样完整抄写出一条
         itemType="imaging" 的记录：conclusion/diagnosis = 该段文字完整原文（多条编号诊断就
         按原文顺序整段抄，不要摘要、不要遗漏任何一条编号），findings = 检查方法/过程描述（如有）
      ③ 这条imaging诊断总结记录和①的lab数值子项**同等重要，缺一不可**——遇到数值表格就只顾着
         提数值、看不到后面还有大段诊断文字没抄，是最常见的遗漏错误，务必检查报告是否还有下文
