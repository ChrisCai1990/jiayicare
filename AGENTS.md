...
> 2026-09-23回退全流程cbc0987a已部署：四端生产构建、版本、健康与Staff/Admin新资源核验通过；仅原金娟试点、无迁移。现有单项协助需明确点击“进入完整就医流程”接入，之后回退直返和留痕生效，不覆盖年度方案。Admin新增质量复核。真实业务未代操作，详见CARE_FLOW_RETURNS_20260923。

> 2026-09-23单项就医协助全流程/定向回退本地完成：修订直返、追加留痕、报告审核及顾问随访审核结束门槛、Admin质量复核；年度方案不覆盖。1300通过/0失败/2跳过，Staff/Admin构建通过。仅金娟、现有任务明确接入、不迁移数据；部署状态见docs/CARE_FLOW_RETURNS_20260923.md。

> 2026-09-23专家沟通展示9c08ea59已部署，11专项/四端构建/生产版本与Staff资源通过；仅金娟、无数据迁移。新提出岗位定向回退与修订原路返回尚未实现，不得称已支持。

> 2026-09-23专家沟通展示本地修订：派单/专员执行页前置顾问原因依据及沟通提示；冻结派单快照优先、缺依据不编造，收集专家反馈。专项测试/Staff1922构建通过，尚未部署。详见ANNUAL_DIRECT_DISPATCH_20260923。

> 2026-09-23直接派单a5b10276已部署：1274通过/0失败/2跳过，四端构建、生产版本/公网健康/Staff新资源核验通过。金娟试点保持、跳过迁移、无真实派单。全托管/旧服务不重复创建，原临床随访保留。详见ANNUAL_DIRECT_DISPATCH_20260923。

> 2026-09-23单项就医协助直接派单：规划师事项/预约/选人员三段展示，去重复关联；专员现场回执/结果、规划师验收独立留痕，确定性任务防重复，顾问原随访不自动完成。全托管/已关联服务保留原流程，不建收费订单。仅金娟、跳过迁移，验证与部署结果见docs/ANNUAL_DIRECT_DISPATCH_20260923.md。

> 2026-09-23分项预约9399a5a0已部署：四端构建/生产HEAD/API及Staff新资源核验通过；金娟白名单保持、跳过迁移，最终40专项通过。未代客户预约或改历史。见ANNUAL_BOOKING_ITEMS_20260923。

> 2026-09-23分项预约修订：门诊/检查分别带顾问科室专家，提前/现场/无需预约逐项留痕，现场待办不伪装完成；关联服务任务显示交接、所属就医专员回填，缺结果不关闭关联需求。1269通过/0失败/2跳过，Staff1917构建；待部署核验。不改历史、仍金娟试点，直接派单页未新增。见docs/ANNUAL_BOOKING_ITEMS_20260923.md。

> 2026-09-23预约排版及顾问权限69b368fc已部署，四端构建/线上版本/公网健康及Staff新资源核验通过；金娟白名单及关闭开关保持、跳过迁移。1258通过/0失败/2跳过。顾问要求只读带入，健管仅日期时间/备注，未代客户预约。详见ANNUAL_ASSISTANCE_BOOKING_20260923。

> 2026-09-23预约排版本地修订：顾问要求逐行只读、依据折叠去重；既定医院科室专家服务端取原计划，健管只填日期时间/备注，医院缺失可补、科室缺失转顾问。执行人员不能编辑取消删除年度scheduled计划；下一处理人显示规划师。22专项与Staff1915构建通过，全量待核验；未部署/客户代操作。详见ANNUAL_ASSISTANCE_BOOKING_20260923。

> 2026-09-23预约入口11223065已部署：19项定向测试/生产四端构建通过，线上版本和公网健康/Staff新资源核验通过。跳过迁移，未改客户记录；刷新重开预约任务可见顾问原计划，真实页面仍待用户验收。

> 2026-09-23预约入口补丁：annualBookingTask被通用openExec误判事务，现首段路由预约详情，顾问plannedContent/content只读带入；年度服务需求同走进度详情，不直接结束。真实入口分支测试通过；用户授权部署，结果随后核验。无客户数据改写。

> 2026-09-23预约与单项进度1104fddb已部署：四端构建/公网健康/Staff新资源通过，GitHub双分支与生产一致。金娟白名单保持，恢复/自动准备关闭，跳过迁移，无客户数据代操作。直接派单页仍未完成，详见docs/ANNUAL_ASSISTANCE_BOOKING_20260923.md。

> 2026-09-23年度协助预约本地修复：单项服务进度不再汇总整年纯提醒；健管待处理仅加入协助预约，独立回执保留原随访。权限/CAS/单项关联及未预约拦截；1253通过/0失败/2跳过，Staff1914构建通过。未部署/改生产。直接派单页尚未替换原服务关联入口，不能称完整闭环，见docs/ANNUAL_ASSISTANCE_BOOKING_20260923.md。

> 2026-09-23补充依据已部署6d4785df：四端构建、公网健康及Staff资源验证通过，生产只读查询已包含已确认checkup研判。金娟白名单及自动扫描关闭保持，跳过迁移；未代客户AI生成/保存。已推送方案仅内部修订草稿，正式修订发布未开放。见docs/ANNUAL_SUPPLEMENT_20260923.md。

> 2026-09-23补充依据入口：默认年度补checkup已确认研判；显式选任意类型已确认研判/已审报告/顾问确认意见生成补充候选，逐项差异应用到未推送草稿；已推送仅留内部修订草稿，不能直接发布或改任务。原PUT保护扩至已推送。1242通过/0失败/2跳过及新增控制器专项6通过，Staff1912构建通过；发布待核验。详见docs/ANNUAL_SUPPLEMENT_20260923.md，取消事项/正式修订发布未开放。

> 2026-09-23focus列表补丁已部署da7e25f0：1237通过/0失败/2跳过，线上版本/公网健康/模块烟测通过；仅后端跳过迁移，金娟白名单不变。未调用真实AI或改客户方案，新输出仍待顾问验收。

> 2026-09-23年度focus失败已只读核实为非空字符串数组，不是客户缺资料；修复列表无损转文字，单次45秒年度模块/来源覆盖校正替代旧focus补正，其他模块原样，严格复验失败不覆盖。规则4，专项11通过；全量/部署结果待核验，未真实AI/客户写入。详见docs/ANNUAL_FOCUS_LIST_FIX_20260923.md。

> 2026-09-23规则3已部署013569ac：四端构建/公网健康及Staff新版资源复核通过，GitHub双分支与生产代码一致；金娟白名单和自动扫描关闭保持，跳过迁移。1233通过/0失败/2跳过，最终专项12通过。未代用户生成/改客户方案，真实AI草稿质量仍待顾问验收。

> 2026-09-23规则3修复：已审逐项报告时间线进入年度生成，日期锚点校验、较旧相关检查解释、1—14天跨模块统筹、年度近期项目去重；客户原始moduleData屏蔽内部时间推理，随访人员只显示姓名且实际派单不变。1233通过/0失败/2跳过，Staff1910模块构建通过；用户授权随后部署，发布结果以核验为准。未改原报告/客户方案或调用真实AI；详见docs/ANNUAL_CLINICAL_RULES_20260923.md。

> 2026-09-23年度生成修复已部署a7e0f89a，1228通过/0失败/2跳过；仅后端发布跳过迁移，公网健康/线上版本及金娟白名单复核通过。年度生成专用Nginx180秒已生效，备份/etc/nginx/annual-ai-backups/jiayicare.20260923002401.bak。未代客户调用AI或改失败快照，首次真实输出仍待用户重试验收。

> 2026-09-23年度生成失败修复：只读定位金娟两次Nginx60秒504，最新AI约71秒后focus为空而校验失败；删除方案无助。有来源的年度体检空正文仅做一次45秒字段补正，其余原样，严格验证字符串正文不接受对象。不触发真实AI/改客户记录。专用年度生成location拟180秒，带配置备份/nginx-t/失败恢复；部署及全量结果待后续核验。

> 2026-09-23已部署59a97f58：含ceb63aff偏好/预约日期与问卷同步，双远端分支/生产一致；1226通过/0失败/2跳过，四端构建、后端健康及公网staff新资源通过。两份基础问卷各追加4道选填，重检无缺项/不重复；未改答卷客户记录，年度金娟白名单/自动扫描关闭保持。真实AI与页面联测仍非本次证据。

> 2026-09-23问卷同步：用户授权全部修改后部署，两份基础问卷补4项选填并映射carePreferences/residence.city，暂不确定不当同意；旧答卷不可覆盖医护修订。显式模板追加脚本（默认预检/CAS/幂等），不改客户答卷或发通知。发布与模板执行结果以随后核验为准，年度金娟白名单不扩。

> 2026-09-22就医偏好/预约日期本地修订：客户基本信息增选填偏好并引用年度AI指纹；新建议日期提前7天派所属健管，旧方案不改期。年度体检统一布局、对象文本逐行。1223通过/0失败/2跳过及后增构建派单测试、1910模块构建通过；未真实AI/页面联测、未部署推送。缺失字段单独AI补全及2–4周/下年度控件未接，医院库不扩；详见docs/CARE_APPOINTMENT_PREFERENCES.md。

> 2026-09-22输出协议修复已部署a1984e9e：GitHub双分支/线上版本一致，四端构建及公网健康/新staff资源通过，金娟白名单及自动任务关闭保持，跳过数据迁移。未调用真实AI或更改失败快照，用户刷新后重试新协议；不能将部署成功当作首次生成质量已验收。

> 2026-09-22生成失败协议修复：线上金娟失败快照只读确认annual_checkup仅旧四字段、缺basisSummary/sourceIds，且输出含禁用monitoring；未调用AI或改快照。白名单改用单一完整动态输出示例替换旧JSON块，规则版本2，来源校验不放松；中文错误持久显示，避免仅短暂toast。1219通过/0失败/2跳过，staff1909模块构建通过；用户授权随后直接部署，保持灰度/跳过迁移。真实AI新协议成功尚待验收。

> 2026-09-22准备清单反馈已部署5706bcc9：1216通过/0失败/2跳过，四端构建及线上版本/健康/staff新资源复核通过。GitHub主干/功能分支同步，金娟白名单及自动准备/恢复关闭保持；跳过数据迁移，不代客户勾选或生成。刷新后普通未确认草稿不再阻断；仅明确必需的未确认研判阻断。

> 2026-09-22准备清单反馈修复：旧自动条件文案从补充说明中剥离，人工额外说明保留；已勾选条件单独显示并实时跟随选择。研判仅requiredCaseReviewIds明确指定且未确认才阻断，未指定草稿不引用、不阻断；缺失必需研判仍阻断，选择按同客户年度范围校验。未保存选择阻止AI/推送。专项7通过、staff1909模块构建通过；全量与部署以随后执行记录为准。用户已授权测试后直接部署，保留金娟白名单、跳过迁移。

> 2026-09-22一致性修订已部署9bae94ac：GitHub两分支与生产版本一致；标准四端构建/后端健康及公网staff新资源通过。金娟白名单、恢复/自动准备/阶段扫描关闭保持，跳过数据迁移。未执行真实AI或代客户确认发布，仍待首次生成质量与重复三次页面验收。

> 2026-09-22年度生成一致性本地修订：白名单生成输入指纹+Mongo原生_id防重/持久复用，模板错配与覆盖遗漏报错不静默删除，来源覆盖供顾问核对；本年度未归档年度/medical/specialty研判自动清单与引用。1213通过/0失败/2跳过及staff1908模块构建通过；最终小补后复验见执行结果。未真实AI验收/部署/客户操作，线上仍2fd133cf；范围与剩余见ANNUAL_GENERATION_CONSISTENCY.md。不能称AI语义绝对正确或跨年相关性已实现。

> 2026-09-22年度表单修订已部署2fd133cf：GitHub master/功能分支与生产一致；标准四端构建、后端健康、公网staff新资源通过。金娟白名单及新恢复/自动准备/阶段扫描关闭复核保持；跳过数据迁移，不改历史日期或客户记录。用户刷新验收，未代做生产业务操作。

> 2026-09-22年度表单五点本地修订：白名单就医/完善检查/复查隐藏标准字段至折叠依据，随访人只读关联客户健管专员，服务安排移至每项末尾并按单项/门诊或体检一站式联动；后端保存固定单次、可信负责人并保留来源及日期。补充说明增高逐行显示，不再自动重复五条件。AI提示同日优先、拆分须来源依据且不得延误明确时限；未实际调用AI或改历史日期。1210测试中1208通过/0失败/2跳过，staff1908模块构建通过；未部署、未改生产客户记录，线上仍4d660de8。
...
> 2026-09-22旧测试收尾：修复11项失效测试，强化实际路由过滤的内存行为校验与住院越权零副作用断言；仅测试/文档修改。相关28项通过，全套1182通过/0失败/2个需独立Mongo配置的用例跳过。使用候选完整依赖NODE_PATH解决旧目录缺uuid，未改运行后端环境。无需再接受11项失败风险；见docs/LEGACY_TEST_REPAIR_20260922.md。未部署/推送/修改生产索引，不冒充真实HTTP/AI全链重跑。

> 2026-09-22发布前补齐：独立候选整合最新生产master43781626，解决5处冲突，保留此前遗漏的基金/报告质量补丁；只读21项唯一索引冲突0，16项缺失待授权新增。新增HEALTH_MANAGEMENT_RECOVERY_ENABLED独立暂停新恢复，旧OCR/提醒保持。五端构建通过；后端1171通过/11失败/2跳过，失败名单与生产基线一致，不称全绿。见docs/RELEASE_CHECKLIST_20260922.md；未推送/部署/改生产索引，不复用旧功能版本直接发布。原运行隔离实例未重载到候选，生产发布仍需明确审批。
