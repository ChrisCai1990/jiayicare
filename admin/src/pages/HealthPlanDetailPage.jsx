...
const TYPE_LABEL = { annual_checkup:'年度体检方案', annual_mgmt:'年度管理方案', nutrition:'营养干预方案', medical_assist:'就医协助方案', tcm:'中医调理方案', rehab:'运动复健方案', psychology:'心理咨询方案' }
...
function AddItemPanel({ plan, onAdded, onCancel }) {
  ...
  const handleSearch = (q) => {
    ...
    searchTimer.current = setTimeout(async () => {
      setSearching(true)
      try {
        const r = await adminAPI.getRequisitionItems(q)
        setResults(r.data || [])
  ...
  const selectItem = (item) => {
    ...
    setForm(f => ({ ...f, name: item.name, category: item.type === 'lab' ? '检验检查' : '影像检查' }))
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await adminAPI.updatePlan(plan._id, { items: [...(plan.items || []), newItem] })
      toast('已添加')
      onAdded()
    ...
    <div style={{ padding: '16px 20px', borderBottom: '1px solid #f0ece4', background: '#f9f7f3' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1.2fr auto', gap: 10, alignItems: 'flex-end' }}>
        ...
        <div ref={dropRef} style={{ position: 'relative' }}>
          ...
          <input className="form-input" placeholder="搜索检验/检查库或手动填写"
            value={searchQ} onChange={e => handleSearch(e.target.value)}
            onFocus={() => results.length && setShowDropdown(true)} autoComplete="off" />
          {searching && <div style={{ position: 'absolute', right: 8, top: 32, fontSize: 11, color: '#aaa' }}>搜索中...</div>}
          {showDropdown && results.length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, background: '#fff', border: '1px solid #e0d9ce', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.12)', maxHeight: 200, overflowY: 'auto' }}>
              {results.map(item => (
...
export default function HealthPlanDetailPage() {
  ...
  const load = async () => {
    try { const r = await adminAPI.getPlan(id); setPlan(r.data) }
    catch { toast('加载失败') }
  ...
  const handlePush = async () => {
    ...
    try { await adminAPI.pushPlan(id); toast('方案已推送'); load() }
    catch (err) { toast(err.message) }
  }

  const handleItemStatus = async (itemId, status) => {
    try { await adminAPI.updatePlanItem(id, itemId, { status }); load() }
    catch (err) { toast(err.message) }
  }

  const handleDelete = async () => {
    if (!window.confirm('确定删除此方案？')) return
    try { await adminAPI.deletePlan(id); toast('已删除'); nav('/health-plan-templates') }
    catch (err) { toast(err.message) }
  }
    ...
    <div className="page">
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button className="btn btn-secondary btn-sm" onClick={() => nav('/health-plan-templates')}>← 返回</button>
          <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <div className="card">
          ...
          <div className="card-body">
            {[
              ['会员', (plan.patientId?.name || '-') + (plan.patientId?.phone ? ' · ' + plan.patientId.phone : '')],
              ['类型', TYPE_LABEL[plan.type]],
              ['状态', STATUS_LABEL[plan.status]],
              ['年度', plan.year + ' 年'],
              ['负责医护', plan.staffId?.name || '（管理员创建）'],
