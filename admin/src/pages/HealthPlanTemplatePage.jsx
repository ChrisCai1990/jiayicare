...
export default function HealthPlanTemplatePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await adminAPI.getPlans({ type: typeFilter, limit: 50 })
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1 className="page-title">健康管理方案</h1>
      ...
      <div className="card">
        ...
        : <table className="table">
            <thead><tr>
              <th>方案名称</th><th>类型</th><th>会员</th><th>负责医护</th><th>状态</th><th>项目数</th><th>创建时间</th><th>操作</th>
            </tr></thead>
            <tbody>
              {plans.map(p => (
                <tr key={p._id} onClick={() => nav(`/health-plan-templates/${p._id}`)} style={{ cursor: 'pointer' }}>
                  ...
                  <td>
                    <button className="btn btn-secondary btn-sm"
                      onClick={e => { e.stopPropagation(); nav(`/health-plan-templates/${p._id}`) }}>查看</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
function NewPlanModal({ onClose, onSaved, nav, defaultType = 'annual_checkup' }) {
  ...
  const handleSubmit = async e => {
    ...
    try { await adminAPI.createPlan(form); onSaved() }
      ...
      <div className="modal">
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            <PatientSearchInput value={form.patientId} onChange={pid => setForm(f => ({ ...f, patientId: pid }))} />
          </div>
          <div className="form-group" style={{ marginBottom: 0 }}>
