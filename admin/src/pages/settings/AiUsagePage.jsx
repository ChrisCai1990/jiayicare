...
export default function AiUsagePage() {
  ...
  const [reportMatches, setReportMatches] = useState(null)
  ...
  return <div className="page aiu-page">
    ...
    {!snapshot ? <p>{busy ? '正在读取用量…' : '用量未加载，请点击刷新重试。'}</p> : <>
      ...
      {tab === 'overview' && <>
        ...
        <section className="aiu-card"><h2>已暂停的报告</h2><p className="aiu-muted">恢复会继续识别并产生费用，已有结果仍需人工审核。累计额度不会重置。</p>{snapshot.pausedReports.length ? snapshot.pausedReports.map(row => <div className="aiu-row" key={row._id}><div><code>{row._id}</code><p>{row.parseJob.message}</p><small className="aiu-muted">{time(row.parseJob.pausedAt)}</small></div><div className="aiu-actions"><button className="btn" disabled={busy} onClick={() => action(async () => { setReport(row._id); setSelectedReport(null); setTab('usage'); const response = await adminAPI.getAiUsage({ reportId: row._id }); setUsage(response.data) })}>查看 / 追加额度</button><button className="btn btn-primary" disabled={busy} onClick={() => action(async () => { await adminAPI.resumeAiReport(row._id); await refresh(); setMessage('任务已恢复排队') })}>恢复识别</button></div></div>) : <p className="aiu-muted">当前没有因预算或异常保护而暂停的报告。</p>}</section>
      </>}
      {tab === 'usage' && <section className="aiu-card"><h2>调用明细</h2><div className="aiu-filter"><input aria-label="报告名称" placeholder="输入报告名称，如血常规、体检报告" value={reportKeyword} onChange={e => setReportKeyword(e.target.value)} /><button className="btn" disabled={busy || !reportKeyword.trim()} onClick={() => action(async () => { const response = await adminAPI.searchAiReports(reportKeyword.trim()); setReportMatches(response.data) })}>搜索报告</button><select aria-label="业务类型" value={business} onChange={e => setBusiness(e.target.value)}><option value="">全部业务</option><option value="ocr">OCR</option><option value="other">其他 AI</option></select><button className="btn btn-primary" disabled={busy} onClick={() => action(() => loadUsage())}>查询</button></div>
        {reportMatches && <div className="aiu-report-results"><p>{reportMatches.rows.length ? '请选择报告（客户 · 日期 · 报告名称）' : '没有找到匹配报告，请换个名称搜索。'}</p>{reportMatches.rows.map(row => <button className="btn" key={row._id} disabled={busy} onClick={() => action(async () => { const response = await adminAPI.getAiUsage({ reportId: row._id, ...(business ? { business } : {}) }); setReport(row._id); setSelectedReport(row); setUsage(response.data); setReportMatches(null) })}>{row.customerName} · {row.date || '日期待补'} · {row.title}</button>)}{reportMatches.hasMore && <p>仅显示前50份，请输入更完整的报告名称。</p>}</div>}
        {report && <p>当前报告：{selectedReport?.title || '已选报告'}{selectedReport && ` · ${selectedReport.customerName} · ${selectedReport.date || '日期待补'}`} <button className="btn" disabled={busy} onClick={() => action(async () => { const response = await adminAPI.getAiUsage(business ? { business } : {}); setReport(''); setSelectedReport(null); setUsage(response.data) })}>查看全部报告</button></p>}
        <div className="aiu-table-wrap"><table><thead><tr>{['时间 / 客户 / 报告', '页码 / 环节', '模型', '实际 Token', '费用', '状态 / 耗时'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{usage.rows.map(row => <tr key={row._id}><td>{time(row.createdAt)}<small>{row.reportId ? `${row.customerName || '未关联客户'} · ${row.reportTitle || '报告已删除或不可用'}` : '其他 AI 调用（未关联报告）'}</small></td><td>{row.page ? `P${row.page}` : '—'}<small>{stageLabels[row.stage] || row.stage}</small></td><td>{row.model}</td><td>{row.actualTokens == null ? '待核对' : number(row.actualTokens)}<small>{row.actualTokens == null ? `预留 ${number(row.reservedTokens)}` : `输入 ${number(row.inputTokens)} / 输出 ${number(row.outputTokens)}`}</small></td><td>{money(row.costMicros)}</td><td>{labels[row.status] || row.status}<small>{row.durationMs == null ? '—' : `${(row.durationMs / 1000).toFixed(1)} 秒`}</small></td></tr>)}</tbody></table>{!usage.rows.length && <p className="aiu-empty">没有符合条件的调用记录。</p>}</div>
