...
export default function HealthFundConfigPage(){
 const toast=useToast(),[form,setForm]=useState(defaults),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[referrals,setReferrals]=useState([]),[products,setProducts]=useState([])
 useEffect(()=>{Promise.all([adminAPI.getHealthFundConfig(),adminAPI.getHealthFundReferrals(),adminAPI.products({status:'on'})]).then(([r,rr,pr])=>{setForm({...defaults,...r.data});setReferrals(rr.data||[]);setProducts(pr.data||[])}).catch(e=>toast(e.message)).finally(()=>setLoading(false))},[])
 ...
 return <div className="page"><div className="page-header"><div><h1 className="page-title">💰 健康基金管理</h1><p className="page-subtitle">设置 C 端实际抵扣规则；企业客户资料及企业专属规则仍在“企业客户管理”中维护。</p></div></div><div className="card"><div className="card-body" style={{maxWidth:860}}>
  ...
  <div className="form-group"><label className="form-label">可抵扣商城产品（未选择表示全部商城产品）</label><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:10,padding:12,border:'1px solid #e2e8f0',borderRadius:10,maxHeight:300,overflowY:'auto'}}>{products.map(p=>{const id=String(p._id);const checked=(form.eligibleProductIds||[]).map(String).includes(id);return <label key={id} style={{display:'flex',gap:8,alignItems:'center',padding:8,background:checked?'#ecfdf5':'#fff',borderRadius:8}}><input type="checkbox" checked={checked} onChange={()=>set('eligibleProductIds',checked?(form.eligibleProductIds||[]).filter(v=>String(v)!==id):[...(form.eligibleProductIds||[]),id])}/><span>{p.name}<small style={{display:'block',color:'#718096'}}>{p.category} · ¥{p.originalPrice}</small></span></label>})}</div>{products.length===0&&<p style={{color:'#718096'}}>暂无已上架商城产品，请先在商城产品管理中上架。</p>}</div>
