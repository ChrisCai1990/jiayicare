...
const defaults={title:'健康基金使用规则',description:'',personalPriority:true,personalDeductionType:'unlimited',personalDeductionValue:0,corporateDeductionType:'percentage',corporateDeductionValue:10,minOrderAmount:0,eligibleCategories:[],eligibleProductIds:[],allowCouponStacking:true,couponDeductionType:'unlimited',couponDeductionValue:0,refundToOriginalSource:true,enabled:false,sharerAmount:0,recipientAmount:0,inviteEnabled:false,inviterAmount:0,inviteeAmount:0,firstLoginEnabled:false,firstLoginAmount:0,pointsExchangeEnabled:true,pointsPerYuan:100,healthCheckinPoints:5}
const typeOptions=[['unlimited','不限制（最多抵至应付金额）'],['fixedAmount','固定金额上限'],['percentage','订单金额比例上限']]
function LimitField({title,type,value,onType,onValue,percentageMax=100}) { return <div className="form-group"><label className="form-label">{title}</label><div style={{display:'grid',gridTemplateColumns:'minmax(240px,1fr) minmax(180px,1fr)',gap:12}}><select className="form-input" value={type} onChange={e=>onType(e.target.value)}>{typeOptions.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select>{type!=='unlimited'&&<div style={{display:'flex',alignItems:'center',gap:8}}><input className="form-input" type="number" min="0" max={type==='percentage'?percentageMax:undefined} value={value} onChange={e=>onValue(e.target.value)}/><span>{type==='percentage'?'%':'元/单'}</span></div>}</div></div> }

const productRuleOptions=[['inherit','继承平台规则（默认10%）'],['percentage','企业赠送基金按订单比例'],['disabled','不享有企业赠送基金抵扣']]
function ProductRuleField({rule,onChange}) {
 ...
 return <div style={{display:'grid',gridTemplateColumns:'minmax(135px,1fr) minmax(95px,.7fr)',gap:8,marginTop:9}} onClick={e=>e.stopPropagation()}>
  ...
  {mode==='percentage'?<div style={{display:'flex',alignItems:'center',gap:5}}><input className="form-input" style={{padding:'7px 8px',minWidth:0}} type="number" min="1" max="10" value={value} onChange={e=>onChange({mode,value:e.target.value})}/><span>%</span></div>:<span/>}
 </div>
}
...
export default function HealthFundConfigPage(){
 ...
 <h3>基金抵扣参数</h3>
  ...
  <div className="form-group"><label className="form-label">企业赠送健康基金平台上限</label><div className="form-input" style={{background:'#f8fafc'}}>订单应付金额的 10%（商品可单独设为1%-10%或不享有）</div></div>
  ...
  <label style={{display:'flex',gap:8,margin:'14px 0'}}><input type="checkbox" checked={form.pointsExchangeEnabled!==false} onChange={e=>set('pointsExchangeEnabled',e.target.checked)}/>每满指定积分自动兑换为1元企业赠送健康基金</label>
  ...
  <p style={{color:'#718096'}}>积分兑换所得计入“企业赠送基金”，按商品设置的1%-10%比例抵扣；现行消费奖励为现金实付每1元获1积分。</p>
