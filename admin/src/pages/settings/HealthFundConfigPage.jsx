...
export default function HealthFundConfigPage(){
 const toast=useToast(),[form,setForm]=useState(defaults),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[referrals,setReferrals]=useState([]),[categories,setCategories]=useState([]),[products,setProducts]=useState([])
 ...
 return <div className="page"><div className="page-header"><div><h1 className="page-title">💰 健康基金管理</h1><p className="page-subtitle">设置 C 端实际抵扣规则；企业客户资料及企业专属规则仍在“企业客户管理”中维护。</p></div></div><div className="card"><div className="card-body" style={{maxWidth:860}}>
  ...
  <div className="form-group"><label className="form-label">具体可抵扣商品（未勾选表示按上方分类全部适用）</label><div style={{maxHeight:430,overflowY:'auto',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:10,padding:14,border:'1px solid #e2e8f0',borderRadius:12}}>{visibleProducts.map(p=>{const id=String(p._id);const checked=(form.eligibleProductIds||[]).map(String).includes(id);return <label key={id} style={{display:'flex',gap:10,alignItems:'center',padding:'11px 12px',background:checked?'#ecfdf5':'#f8fafc',borderRadius:9,border:checked?'1px solid #22A06B':'1px solid transparent'}}><input type="checkbox" checked={checked} onChange={()=>set('eligibleProductIds',checked?(form.eligibleProductIds||[]).filter(v=>String(v)!==id):[...(form.eligibleProductIds||[]),id])}/><span><strong>{p.name}</strong><small style={{display:'block',color:'#718096'}}>{p.category||'未分类'} · ¥{p.price}</small></span></label>})}{visibleProducts.length===0&&<p style={{color:'#718096'}}>当前分类下暂无上架商品。</p>}</div></div>
