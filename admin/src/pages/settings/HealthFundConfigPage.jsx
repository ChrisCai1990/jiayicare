...
const defaults={title:'健康基金使用规则',description:'',personalPriority:true,personalDeductionType:'unlimited',personalDeductionValue:0,corporateDeductionType:'fixedAmount',corporateDeductionValue:200,minOrderAmount:0,eligibleCategories:[],eligibleProductIds:[],allowCouponStacking:true,couponDeductionType:'unlimited',couponDeductionValue:0,refundToOriginalSource:true,enabled:false,sharerAmount:0,recipientAmount:0,inviteEnabled:false,inviterAmount:0,inviteeAmount:0,firstLoginEnabled:false,firstLoginAmount:0}
...
export default function HealthFundConfigPage(){
 const toast=useToast(),[form,setForm]=useState(defaults),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[referrals,setReferrals]=useState([]),[categories,setCategories]=useState([])
 useEffect(()=>{Promise.all([adminAPI.getHealthFundConfig(),adminAPI.getHealthFundReferrals(),adminAPI.productCategories()]).then(([r,rr,cr])=>{setForm({...defaults,...r.data});setReferrals(rr.data||[]);setCategories((cr.data||[]).filter(c=>c.status!=='disabled'))}).catch(e=>toast(e.message)).finally(()=>setLoading(false))},[])
 const set=(k,v)=>setForm(f=>({...f,[k]:v})); const save=async()=>{setSaving(true);try{const r=await adminAPI.updateHealthFundConfig({...form,eligibleProductIds:[]});setForm({...defaults,...r.data});toast(r.message)}catch(e){toast(e.message)}finally{setSaving(false)}}
 ...
 return <div className="page"><div className="page-header"><div><h1 className="page-title">💰 健康基金管理</h1><p className="page-subtitle">设置 C 端实际抵扣规则；企业客户资料及企业专属规则仍在“企业客户管理”中维护。</p></div></div><div className="card"><div className="card-body" style={{maxWidth:860}}>
  ...
  <div className="form-group"><label className="form-label">可抵扣商城分类（未选择表示全部商城产品）</label><p style={{color:'#718096',margin:'4px 0 10px'}}>分类直接读取商城；以后新增到已选分类的商品会自动适用，无需再次配置。</p><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:10,padding:14,border:'1px solid #e2e8f0',borderRadius:12}}>{categories.map(c=>{const name=c.name;const checked=(form.eligibleCategories||[]).includes(name);return <label key={c._id||name} style={{display:'flex',gap:9,alignItems:'center',padding:'11px 12px',background:checked?'#ecfdf5':'#f8fafc',borderRadius:9,border:checked?'1px solid #22A06B':'1px solid transparent'}}><input type="checkbox" checked={checked} onChange={()=>set('eligibleCategories',checked?(form.eligibleCategories||[]).filter(v=>v!==name):[...(form.eligibleCategories||[]),name])}/><span>{name}</span></label>})}</div>{categories.length===0&&<p style={{color:'#718096'}}>商城暂无分类，请先在商城分类管理中建立分类。</p>}</div>
