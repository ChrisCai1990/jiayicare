...
export default function SupplyWorkflowConfigPage() {
  ...
  const patchProduct = (id, patch) => setProducts(prev => prev.map(p => p._id === id ? { ...p, serviceWorkflow: { key: '', followUpPlanId: null, followUpPlanIds: [], notes: '', ...(p.serviceWorkflow || {}), ...patch } } : p))
  const saveProduct = async product => { setSavingProduct(product._id); try { const w = product.serviceWorkflow || {}; const ids = w.followUpPlanIds?.length ? w.followUpPlanIds : (w.followUpPlanId ? [typeof w.followUpPlanId === 'object' ? w.followUpPlanId._id : w.followUpPlanId] : []); const r = await adminAPI.updateProductServiceWorkflow(product._id, { key: w.key || '', followUpPlanIds: ids, notes: w.notes || '' }); patchProduct(product._id, r.data); toast(`${product.name}：${r.message}`) } catch (e) { toast(e.message) } finally { setSavingProduct('') } }
