...
const FLOW_OPTIONS = [['', '尚未关联'], ['annual_management', '年度健康管理'], ['health_record_management', '健康档案管理'], ['health_assessment', '健康评估与复查'], ['nutrition_intervention', '营养干预'], ['checkup', '体检／检查管理'], ['medical_assist', '就医协助'], ['medical_proxy', '医疗代诊服务'], ['rehab', '运动复健'], ['tcm', '中医调理'], ['psychology', '心理支持'], ['medication_supply', '药品定期配取'], ['supplement_supply', '营养素定期补充'], ['generic_followup', '通用随访'], ['fulfillment_only', '单次履约／核销']]
const FLOW_CARDS = [
  ...
  ['医疗代诊服务', '规划师核对诉求并指导上传 → 健管审核资料 → 健康顾问确认方案 → 规划师复核指派 → 健管预约专家 → 就医专员代诊上传病历 → 健康顾问审核随访；规划师全程督办', '/products', '管理服务产品'],
...
const PRODUCT_STAGE_TEXT = {
  '体检一站式服务': '健康规划师总督办 → 用户填写健康文件 → 健康顾问定方案 → 健康规划师预约 → 就医专员现场执行 → 健管收报告并审核 → 健康顾问结果评估和随访计划 → 健康规划师最终验收关闭',
  '门诊一站式服务': '健康规划师总督办 → 健管收资料 → 健康顾问评估 → 健管约诊 → 健康规划师安排人员 → 就医专员代诊开单 → 就医专员陪诊归档 → 健康规划师最终验收关闭',
  '医疗代诊服务': '健康规划师核对诉求并指导客户上传/选择资料 → 健管专员审核 → 健康顾问确认医院、科室、专家、目标及交流内容 → 健康规划师复核并预指派 → 健管专员预约专家门诊 → 就医专员代诊并上传病历 → 健康顾问确认随访；健康规划师持续督办至代诊结束',
}
