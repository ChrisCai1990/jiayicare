...
const EMPTY = { name: '', mnemonic: '', costPrice: 0, retailPrice: 0, unit: '次', categoryId: '', orders: [] }
...
export default function LabTestPackagePage() {
  ...
  useEffect(() => {
    adminAPI.labTestOrders({ limit: 200 }).then(r => setAllOrders(r.data)).catch(() => {})
  }, [])
  ...
  const openEdit = item => {
    ...
    setForm({
      ...
      orders: (item.orders || []).map(o => o._id || o),
    })
  ...
  const toggleOrder = id => setForm(f => ({
    ...f, orders: f.orders.includes(id) ? f.orders.filter(i => i !== id) : [...f.orders, id],
  }))
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700 }}>检验套餐</h2>
          <p style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>一组检验医嘱的组合，如"年度基础体检包"</p>
        </div>
            ...
            <tbody>
              {list.map(item => (
                <tr key={item._id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                  ...
                  <td style={{ padding: '10px 14px', color: '#6B7280' }}>{item.orders?.length || 0} 项</td>
        ...
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowModal(false) }}>
          <div className="modal" style={{ maxWidth: 520 }}>
            <div className="modal-header">
              <h3 className="modal-title">{editId ? '编辑检验套餐' : '新增检验套餐'}</h3>
            ...
            <div className="modal-body" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div className="form-group" style={{ marginBottom: 0, gridColumn: 'span 2' }}>
                  ...
                  <input className="form-input" value={form.name} onChange={set('name')} placeholder="如：年度基础体检包" />
                </div>
              ...
              <div style={{ marginTop: 16 }}>
                <label className="form-label">包含检验医嘱（多选）</label>
                <div style={{ border: '1px solid #E5E7EB', borderRadius: 8, maxHeight: 200, overflowY: 'auto', padding: 8 }}>
                  {allOrders.length === 0 ? <div style={{ color: '#aaa', fontSize: 13, padding: 8 }}>暂无检验医嘱</div>
                    : allOrders.map(o => (
                    <label key={o._id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 8px', cursor: 'pointer' }}>
                      <input type="checkbox" checked={form.orders.includes(o._id)} onChange={() => toggleOrder(o._id)} />
                      <span style={{ fontSize: 13 }}>{o.name}</span>
                    </label>
                  ))}
                </div>
                {form.orders.length > 0 && <div style={{ fontSize: 12, color: '#1E6B50', marginTop: 4 }}>已选 {form.orders.length} 项</div>}
              </div>
            </div>
