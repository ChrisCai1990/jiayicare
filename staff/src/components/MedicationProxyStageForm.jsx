...
const FIELDS = [
  ...
  ['singleDose', '单次服用剂量'], ['totalQuantity', '配备总量'], ['dailyQuantity', '每日服用总量（用于计算下次配药）'],
  ['institution', '配药机构'], ['department', '配药科室'], ['expert', '配药专家'],
]
...
export default function MedicationProxyStageForm({ task, value, onChange }) {
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    ...
    {stage === 'intake' ? <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>{FIELDS.slice(0, 7).map(([key, title]) => field(key, title))}</div>
      <label style={label}>配药机构类型<select className="form-control" value={data.institutionType || ''} onChange={e => update({ institutionType: e.target.value })}><option value="">请选择</option><option value="hospital">医院</option><option value="pharmacy">线下药房</option><option value="online">线上采购</option></select></label>
      ...
      {['advisor', 'review', 'booking'].includes(stage) && <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>{FIELDS.slice(7).map(([key, title]) => field(key, title))}</div>}
