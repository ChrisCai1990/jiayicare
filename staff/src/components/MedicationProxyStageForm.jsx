...
export default function MedicationProxyStageForm({ task, value, onChange, staffList = [] }) {
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    ...
    {stage === 'progress' ? <div style={{ padding: 14, borderRadius: 10, background: '#EFF8F4', border: '1px solid #B9DDD0' }}><b>当前进度：{({ advisor: '健康顾问评估中', booking: '健管专员预约中', planner: '待健康规划师分配执行人员', execute: '就医专员配药及配送中' })[data.currentStage] || '处理中'}</b><div style={{ marginTop: 6, fontSize: 12, color: '#65776F' }}>仅供健康规划师查看；完成配药、客户确认和配送安排后自动结束。</div></div> : stage === 'intake' ? <>
      ...
      {stage === 'booking' && <><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}><label style={label}>预约日期<input type="date" className="form-control" value={data.appointmentDate || ''} onChange={e => update({ appointmentDate: e.target.value })} /></label><label style={label}>预约时间<input type="time" className="form-control" value={data.appointmentTime || ''} onChange={e => update({ appointmentTime: e.target.value })} /></label></div><div style={{ fontSize: 12, color: '#4A6558' }}>就医专员由健康规划师提前指定；确认预约后直接转交该专员。</div></>}
