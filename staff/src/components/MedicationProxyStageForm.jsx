...
const appointmentSlots = data => Array.isArray(data?.appointmentSlots) && data.appointmentSlots.length
  ...
  : [{ department: data?.department || '', expert: data?.expert || '', appointmentDate: data?.appointmentDate || '', appointmentTime: data?.appointmentTime || '' }]
...
export default function MedicationProxyStageForm({ task, value, onChange, staffList = [] }) {
  ...
  const assistantField = required => <label style={label}>本单就医专员{required ? ' *' : '（可选）'}<select className="form-control" value={data.medicalAssistantId || ''} onChange={e => { const selected = staffList.find(item => String(item._id) === e.target.value); update({ medicalAssistantId: e.target.value, medicalAssistantName: selected?.name || '' }) }}><option value="">{required ? '请选择就医专员' : '暂不指定，由健康规划师后续分配'}</option>{staffList.filter(item => item.role === 'medicalAssistant' && item.staffStatus !== 'inactive').map(item => <option key={item._id} value={item._id}>{item.name}{item.title ? ` · ${item.title}` : ''}</option>)}</select></label>
  return <div style={{ display: 'grid', gap: 12 }}>
    ...
    {stage === 'progress' ? <div style={{ padding: 14, borderRadius: 10, background: '#EFF8F4', border: '1px solid #B9DDD0' }}><b>当前进度：{({ advisor: '健康顾问评估中', booking: '健管专员预约中', planner: '待健康规划师分配执行人员', execute: '就医专员配药及配送中' })[data.currentStage] || '处理中'}</b><div style={{ marginTop: 6, fontSize: 12, color: '#65776F' }}>仅供健康规划师查看；完成配药、客户确认和配送安排后自动结束。</div></div> : stage === 'intake' ? <>
      ...
      <div style={{ fontSize: 13, lineHeight: 1.8 }}>药品：{data.intakeSnapshot?.brandName || data.brandName}（{data.intakeSnapshot?.chemicalName || data.chemicalName}）　规格：{data.intakeSnapshot?.specification || data.specification}<br />剂量：{data.intakeSnapshot?.singleDose || data.singleDose}　总量：{data.intakeSnapshot?.totalQuantity || data.totalQuantity}　机构：{data.intakeSnapshot?.institution || data.institution}<br />本单就医专员：{data.medicalAssistantName || data.intakeSnapshot?.medicalAssistantName || '未指定'}</div>
      ...
      {['advisor', 'review'].includes(stage) && <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>{FIELDS.slice(6).map(([key, title]) => field(key, title))}</div>}
      ...
      {stage === 'booking' && <>{appointmentSlots(data).map((row, index) => <div key={index} style={{ display: 'grid', gap: 8, padding: 10, border: '1px solid #DCE8E2', borderRadius: 8 }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><strong>配药预约 {index + 1}</strong>{index > 0 && <button type="button" className="btn btn-secondary btn-sm" onClick={() => update({ appointmentSlots: appointmentSlots(data).filter((_, i) => i !== index) })}>移除</button>}</div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>{[['department', '科室'], ['expert', '专家/医生'], ['appointmentDate', '预约日期'], ['appointmentTime', '预约时间']].map(([key, title]) => <label key={key} style={label}>{title}<input className="form-control" type={key === 'appointmentDate' ? 'date' : key === 'appointmentTime' ? 'time' : 'text'} value={row[key] || ''} onChange={e => { const slots = appointmentSlots(data).map((item, i) => i === index ? { ...item, [key]: e.target.value } : item); update({ appointmentSlots: slots, ...(index === 0 ? { department: slots[0].department, expert: slots[0].expert, appointmentDate: slots[0].appointmentDate, appointmentTime: slots[0].appointmentTime } : {}) }) }} /></label>)}</div></div>)}<button type="button" className="btn btn-secondary btn-sm" onClick={() => update({ appointmentSlots: [...appointmentSlots(data), { department: '', expert: '', appointmentDate: '', appointmentTime: '' }] })}>＋ 新增配药预约</button><div style={{ fontSize: 12, color: '#4A6558' }}>就医专员由健康规划师提前指定；确认全部预约后直接转交该专员。</div></>}
      {stage === 'execute' && data.bookingSnapshot?.appointmentSlots?.length > 1 && <div>{data.bookingSnapshot.appointmentSlots.map((row, index) => <div key={index}>预约 {index + 1}：{row.department} {row.expert} · {row.appointmentDate} {row.appointmentTime}</div>)}</div>}
      {stage === 'planner' && <><div style={{ fontSize: 12, color: '#4A6558' }}>本单尚未指定执行人员，请健康规划师分配后再转交配药执行。</div>{assistantField(true)}</>}
