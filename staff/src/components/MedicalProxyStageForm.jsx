...
export default function MedicalProxyStageForm({ task, value = {}, onChange, reports = [], staffList = [], onOpenReport }) {
  ...
  const escortSummary = <div style={{ display: 'grid', gap: 7, background: '#F5F8F6', border: '1px solid #DCE8E1', borderRadius: 8, padding: 12, fontSize: 13 }}>
    ...
    {escortPlan.escortCategory === 'exam' && Array.isArray(escortPlan.escortExams) && escortPlan.escortExams.length ? escortPlan.escortExams.map((exam, index) => <div key={index} style={{ padding: 8, border: '1px solid #DCE8E1', borderRadius: 6 }}><strong>检查 {index + 1}：{exam.item}</strong><div>检查科室/地点：{exam.department || '待确认'}{exam.expert ? ` · 专家：${exam.expert}` : ''}{exam.time ? ` · 时间：${exam.time}` : ''}</div><div>检查前注意事项：{exam.precautions || '待与检查机构核实'}</div></div>) : Array.isArray(escortPlan.escortDepartments) && escortPlan.escortDepartments.length ? escortPlan.escortDepartments.map((item, index) => <div key={index}>就诊安排 {index + 1}：{item.department || '未填写科室'}{item.expert ? ` · 专家：${item.expert}` : ''}{item.time ? ` · 到诊：${item.time}` : ''}</div>) : <><div>科室：{escortPlan.department || '未填写'}</div>{escortPlan.expert && <div>专家：{escortPlan.expert}</div>}</>}
