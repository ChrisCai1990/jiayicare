...
export default function CheckupManagerReviewForm({ task, value, onChange, onOpenReport }) {
  ...
  const removeAssociation = (kind, item, id) => {
    if (kind === 'record') update({ medicalRecordIds: data.medicalRecordIds.filter(value => value !== id), aiGenerated: false })
    else updateAssignments({ ...data.reportAssignments, [item]: (data.reportAssignments[item] || []).filter(value => value !== id) })
  }
  ...
  const files = (ids, title, kind, item) => <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', fontSize: 12 }}><span style={{ color: ids.length ? '#1E6B50' : '#B45309' }}>{ids.length ? `已关联 ${ids.length} 份` : '尚未上传'}</span>{ids.map((id, index) => <React.Fragment key={id}><button type="button" className="btn btn-secondary btn-sm" onClick={() => onOpenReport?.(id, `${title} ${index + 1}`)}>查看资料 {index + 1}</button><button type="button" className="btn btn-danger btn-sm" onClick={() => removeAssociation(kind, item, id)}>移除</button></React.Fragment>)}</div>
  const uploader = (kind, title, description, ids, item = '', required = true) => <section style={{ border: '1px solid #D8E7DF', borderRadius: 10, padding: 13, display: 'grid', gap: 8 }}><div><b>{title}{required ? ' *' : ''}</b><div style={{ fontSize: 12, color: '#65776F', marginTop: 3 }}>{description}</div></div><label className="btn btn-secondary" style={{ width: 'fit-content', cursor: uploading ? 'wait' : 'pointer' }}>{uploading === `${kind}:${item}` ? '上传中…' : `上传${title}`}<input type="file" accept="image/*,.pdf" disabled={!!uploading} onChange={event => upload(event, kind, item)} style={{ display: 'none' }} /></label>{files(ids, title, kind, item)}</section>
  return <div style={{ display: 'grid', gap: 14 }}>
    <section style={{ padding: 12, borderRadius: 9, background: '#FFF8ED', fontSize: 13, lineHeight: 1.7 }}><b>资料收集与审核：</b>{intake.institution || '检查机构待确认'}<br />检查报告必须逐项对应本次检查项目；逐项报告齐全后，AI 将结合可选门诊病历生成随访计划草稿并转健康顾问审核。</section>
    ...
    {uploader('record', '门诊病历（选填）', '客户如有本次专家看诊病历或病程记录，可在此上传；缺失不阻断审核。', data.medicalRecordIds, '', false)}
