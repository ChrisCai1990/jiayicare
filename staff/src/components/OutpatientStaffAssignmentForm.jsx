...
export default function OutpatientStaffAssignmentForm({ task, value, onChange, staffList = [] }) {
  ...
  return <div style={{ display: 'grid', gap: 14 }}>
    ...
    <section style={{ border: '1px solid #E8DCC8', borderRadius: 10, background: '#FFFAF2', padding: 13, display: 'grid', gap: 8 }}>
      ...
      {(Array.isArray(booking.postCheckAppointments) ? booking.postCheckAppointments : (booking.postCheckAppointment ? [booking.postCheckAppointment] : [])).map((row, index) => <div key={`expert-${index}`} style={{ fontSize: 13 }}><span style={{ color: '#8A7656' }}>检查后专家门诊{index + 1}：</span>{[row.department, row.expertName].filter(Boolean).join(' · ') || '科室/专家待确认'} · {scheduleText(row)}</div>)}
