...
const expertVisitsFrom = booking => Array.isArray(booking?.postCheckAppointments) ? booking.postCheckAppointments : (booking?.postCheckAppointment ? [booking.postCheckAppointment] : [])
const inspectionDateFrom = booking => booking?.specialCheckAppointments?.find(row => row.appointmentDate)?.appointmentDate || expertVisitsFrom(booking)[0]?.appointmentDate || ''
...
export const validateOutpatientProxyVisit = value => {
  ...
  const expert = [...expertVisitsFrom(value.bookingSnapshot)].sort((a, b) => scheduleText(a).localeCompare(scheduleText(b)))[0]
