...
export const validateOutpatientProxyVisit = value => {
  ...
  if (!value?.proxyVisitResult?.trim()) return '请填写代诊实际结果'
  if (!value?.examOrderSummary?.trim()) return '请填写取得的检查单/检查项目'
...
export default function OutpatientProxyVisitForm({ task, value, onChange }) {
  ...
  return <div style={{ display: 'grid', gap: 14 }}>
    ...
    <label style={labelStyle}>实际取得的全部检查单/检查项目 *<textarea className="form-control" rows={3} value={data.examOrderSummary} onChange={e => update({ examOrderSummary: e.target.value })} /></label>
