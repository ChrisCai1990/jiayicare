...
import CareFlowReviewEvidence from './CareFlowReviewEvidence'
import CareFlowReportUploads from './CareFlowReportUploads'
...
export default function CareFlowCard({task,staff,initialData=null,onCompleted}){
  ...
  const [uploadBusy,setUploadBusy]=useState(false),[uploadPending,setUploadPending]=useState(false)
  ...
  return <section ref={topRef} style={{display:'grid',gap:16,fontSize:14,lineHeight:1.6}}>
    ...
    {s.mode!=='reminder'&&<section style={{background:'#F6FBF8',padding:12}}><h4>就医目的与专家沟通（本次顾问交接）</h4><div style={{whiteSpace:'pre-wrap'}}>{brief.reason||'就医原因未明确，请退回顾问补充。'}{brief.basis&&`\n依据：${brief.basis}`}{brief.communication&&`\n明确沟通要求：${brief.communication}`}</div><p>向专家说明上述目的与既往检查记录，请专家评估本次项目：{brief.items||'待顾问明确'}。记录专家意见、开单和后续要求，不自行新增医疗判断。</p></section>}
    ...
    {mine&&stage!=='closed'&&<>
      ...
      {stage==='review'&&!s.draftStale&&<><CareFlowReviewEvidence reports={s.data.audit?.reports}/>{input('content','审核修订随访计划',8,s.data.draft?.content||'')}<label>随访日期<input type="date" className="form-input" value={get('date',s.data.draft?.date||'')} onChange={e=>set('date',e.target.value)}/></label>{input('note','顾问审核意见')}</>}
      ...
      {stage!=='draft'&&!(stage==='review'&&s.draftStale)&&<><label><input type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/> 我已核对本次交接及修订内容</label><button className="btn btn-primary" disabled={busy||uploadBusy||uploadPending||!confirmed||(s.returns?.length>0&&!correction.trim())} onClick={complete}>{s.returns?.length?'修订完成，直接返回发起环节':stage==='review'?'审核通过，生成随访任务并结束服务':stage==='planner'?'确认派单，交就医专员':'完成本环节，交下一步'}</button></>}
    </>}
    ...
    <details><summary>完整流转及修订记录（{data.events.length}）</summary>{data.events.map((e,i)=><div key={i} style={{borderBottom:'1px solid #ddd',padding:10}}><b>{config.labels[e.stage]||e.stage} · {({start:'接入',reminder_visit_completed:'已就医，转资料收集',complete:'完成交接',return:'回退',correct:'修订直返',upload:'上传',draft_requested:'生成草稿',draft_generated:'草稿生成完成',draft_failed:'生成未完成',quality_review:'质量复核',upload_owner_updated:'报告承接岗位调整',customer_report_uploaded:'客户上传资料',customer_upload_completed:'客户确认资料已上传完毕'})[e.action]||e.action}</b><div>{e.name||e.role||'系统'} · {new Date(e.at).toLocaleString('zh-CN')}</div>{e.action==='upload_owner_updated'&&<div>{e.reason}（原负责人：{e.previousAssignee}；现负责人：{e.assignedTo}）</div>}{e.action==='customer_report_uploaded'&&<div>资料：{e.title}</div>}{e.action==='customer_upload_completed'&&<div>本次共 {e.reportIds?.length||0} 份资料，仍需健管审核。</div>}{e.reason&&e.action!=='upload_owner_updated'&&<div>问题：{e.reason}；责任环节：{config.labels[e.targetStage]} · {e.targetName}（归因待复核）</div>}{e.correction&&<div>修订：{e.correction}；返回{config.labels[e.returnTo]}；耗时{Math.round(e.durationMs/60000)}分钟</div>}{(e.before||e.after)&&<details><summary>修订前后原文</summary><pre style={{whiteSpace:'pre-wrap',wordBreak:'break-word'}}>{JSON.stringify({修订前:e.before,修订后:e.after},null,2)}</pre></details>}</div>)}</details>
