...
export default function CareFlowCard({task,staff,initialData=null}){
  ...
  return <section ref={topRef} style={{display:'grid',gap:16,fontSize:14,lineHeight:1.6}}>
    ...
    {mine&&stage!=='closed'&&<>
      ...
      {stage==='upload'&&<><label>资料类型<select className="form-input" value={category} onChange={e=>setCategory(e.target.value)}><option value="outpatient_record">门诊病历</option><option value="prescription_order">处方/医嘱单</option><option value="exam_report">检查报告</option></select></label><input className="form-input" placeholder="资料名称" value={fileTitle} onChange={e=>setFileTitle(e.target.value)}/><input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={e=>setFile(e.target.files?.[0])}/><button disabled={busy||!file||!fileTitle.trim()} onClick={()=>act(()=>{const body=new FormData();body.append('file',file);body.append('title',fileTitle);body.append('category',category);return careFlowAPI.upload(data._id,body)})}>上传并留存原件</button>{data.reports.map(r=><label key={r._id}><input type="checkbox" checked={get('reportIds',s.data.upload?.reportIds||[]).includes(String(r._id))} onChange={e=>set('reportIds',e.target.checked?[...get('reportIds',s.data.upload?.reportIds||[]),String(r._id)]:get('reportIds',s.data.upload?.reportIds||[]).filter(v=>v!==String(r._id)))}/>{r.title}（取消仅移出本次交接，原件保留）</label>)}{input('note','资料完整性说明、尚缺内容或补传说明')}</>}
