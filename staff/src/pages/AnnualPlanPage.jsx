...
const LIFESTYLE_DIMENSIONS = [
  { key: 'diet',             label: '饮食习惯', questKey: 'diet' },
  { key: 'exercise',         label: '运动状况', questKey: 'exercise' },
  { key: 'sleep',            label: '睡眠质量', questKey: 'sleep' },
  { key: 'smoking_drinking', label: '烟酒情况', questKey: 'smoking' },
  { key: 'stress',           label: '心理压力', questKey: 'stress' },
]

// ── 工具函数 ──────────────────────────────────────────────────────────
function getModuleData(moduleData, moduleKey) {
...
function setModuleField(moduleData, moduleKey, field, value) {
  return {
    ...moduleData,
    [moduleKey]: {
      ...getModuleData(moduleData, moduleKey),
      [field]: value,
    },
  }
}
...
function ModulePanel({ module: mod, moduleData, onChange, patientId, nav }) {
  ...
  const enabled = data.enabled !== false // 默认开启
    ...
    <div style={{
      ...
      {open && enabled && (
        <div style={{ padding: '0 18px 18px', borderTop: '1px solid #F0EDE7' }}>
          {mod.key === 'medical_treatment' && (
            <MedicalTreatmentFields data={data} set={set} nav={nav} patientId={patientId} />
          )}
          {mod.key === 'specialist_collab' && (
            <SpecialistCollabFields data={data} set={set} nav={nav} patientId={patientId} />
          )}
          {mod.key === 'daily_monitoring' && (
            <DailyMonitoringFields data={data} set={set} nav={nav} patientId={patientId} />
          )}
          {mod.key === 'vaccine' && (
            <VaccineFields data={data} set={set} />
          )}
          {mod.key === 'lifestyle' && (
            <LifestyleFields data={data} set={set} nav={nav} patientId={patientId} />
          )}
          {mod.key === 'checkup' && (
  ...
  if (multiline) {
    return (
      <textarea
        ...
        rows={3}
        style={{ width: '100%', padding: '7px 10px', border: '1px solid #E0D9CE', borderRadius: 8, fontSize: 13, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
      />
    )
  }
  return (
...
function ActionLink({ label, onClick }) {
  return (
    <button
      onClick={onClick}
      style={{
        background: 'none', border: '1px solid #1E6B50', color: '#1E6B50',
...
// ── 医疗问题解决模块 ──────────────────────────────────────────────────
function MedicalTreatmentFields({ data, set, nav, patientId }) {
  return (
    <div style={{ paddingTop: 8 }}>
      <FieldRow label="就医频率">
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ flex: 1 }}>
            <SelectInput value={data.time} onChange={v => set('time', v)} options={TIME_OPTIONS} placeholder="请选择频率" />
          </div>
          <ActionLink label="新建随访" onClick={() => nav(`/followups`)} />
        </div>
      </FieldRow>
      <FieldRow label="就诊医院">
        <SelectInput value={data.hospital} onChange={v => set('hospital', v)} options={HOSPITAL_OPTIONS} placeholder="请选择医院" />
      </FieldRow>
      <FieldRow label="就诊科室">
...
// ── 全专联合模块 ──────────────────────────────────────────────────────
function SpecialistCollabFields({ data, set, nav, patientId }) {
  return (
    <div style={{ paddingTop: 8 }}>
      <FieldRow label="会诊计划日期">
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ flex: 1 }}>
            <DateInput value={data.date} onChange={v => set('date', v)} />
          </div>
          <ActionLink label="新建随访" onClick={() => nav(`/followups`)} />
        </div>
      </FieldRow>
      <FieldRow label="会诊医院">
        <TextInput value={data.hospital} onChange={v => set('hospital', v)} placeholder="医院名称" />
      </FieldRow>
      <FieldRow label="科室/专业">
        <TextInput value={data.department} onChange={v => set('department', v)} placeholder="如：多学科联合门诊（MDT）" />
      </FieldRow>
      <FieldRow label="专家姓名">
        <TextInput value={data.doctor} onChange={v => set('doctor', v)} placeholder="参与会诊的专家姓名" />
      </FieldRow>
      <FieldRow label="会诊目的">
...
// ── 日常监测模块 ──────────────────────────────────────────────────────
function DailyMonitoringFields({ data, set, nav, patientId }) {
  return (
    <div style={{ paddingTop: 8 }}>
      {MONITORING_ITEMS.map(item => (
        <FieldRow key={item.key} label={item.label}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            {item.key === 'bp_weight' && !!data[item.key] && (
              <ActionLink label="新建随访" onClick={() => nav(`/followups`)} />
            )}
          </div>
        </FieldRow>
      ...
      <FieldRow label="监测频率">
        <SelectInput value={data.frequency} onChange={v => set('frequency', v)} options={TIME_OPTIONS} placeholder="请选择频率" />
      </FieldRow>
      <FieldRow label="其他监测项目">
        <TextInput value={data.other} onChange={v => set('other', v)} placeholder="自定义监测项目说明" />
      </FieldRow>
      <FieldRow label="备注">
        <TextInput value={data.notes} onChange={v => set('notes', v)} placeholder="监测执行说明或特殊要求" multiline />
      </FieldRow>
    </div>
  )
}
...
function VaccineFields({ data, set }) {
  return (
    <div style={{ paddingTop: 8 }}>
        ...
        <FieldRow key={item.key} label={item.label}>
          ...
          {!!data[item.key] && (
            <div style={{ marginTop: 8, paddingLeft: 46, display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <div style={{ flex: 1 }}>
                <DateInput value={data[`${item.key}_date`]} onChange={v => set(`${item.key}_date`, v)} />
              </div>
            </div>
          )}
        </FieldRow>
      ...
      <FieldRow label="其他疫苗">
        <TextInput value={data.other} onChange={v => set('other', v)} placeholder="其他需接种的疫苗" />
      </FieldRow>
      <FieldRow label="备注">
        <TextInput value={data.notes} onChange={v => set('notes', v)} placeholder="接种禁忌或注意事项" multiline />
      </FieldRow>
    </div>
  )
}
...
// ── 生活方式模块 ──────────────────────────────────────────────────────
function LifestyleFields({ data, set, nav, patientId }) {
  return (
    <div style={{ paddingTop: 8 }}>
      ...
      {LIFESTYLE_DIMENSIONS.map(dim => (
        <FieldRow key={dim.key} label={dim.label}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Toggle
              value={!!data[`${dim.key}_assessed`]}
              onChange={v => set(`${dim.key}_assessed`, v)}
              label="纳入评估"
            />
            <ActionLink label="发送问卷" onClick={() => nav(`/questionnaires`)} />
            {data[`${dim.key}_score`] && (
              <span style={{ fontSize: 12, background: '#E8F5EF', color: '#1E6B50', padding: '2px 8px', borderRadius: 10 }}>
      ...
      <FieldRow label="干预目标">
        <TextInput value={data.goal} onChange={v => set('goal', v)} placeholder="期望改善的生活方式目标" multiline />
      </FieldRow>
      <FieldRow label="干预计划">
        <TextInput value={data.plan} onChange={v => set('plan', v)} placeholder="具体干预措施和时间安排" multiline />
      </FieldRow>
      <FieldRow label="备注">
        <TextInput value={data.notes} onChange={v => set('notes', v)} placeholder="其他说明" multiline />
      </FieldRow>
...
// ── 体检方案模块 ──────────────────────────────────────────────────────
function CheckupFields({ data, set, nav, patientId }) {
  return (
    <div style={{ paddingTop: 8 }}>
      {/* 功能医学检测 */}
      <FieldRow label="功能医学检测">
        ...
        {!!data.functional_medicine && (
          <div style={{ marginTop: 8 }}>
            <TextInput value={data.functional_medicine_items} onChange={v => set('functional_medicine_items', v)} placeholder="具体检测项目（可多项）" />
          </div>
        )}
      </FieldRow>
      ...
      <FieldRow label="年度体检套餐">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <ActionLink label="关联健康方案" onClick={() => nav(`/patients/${patientId}`)} />
        </div>
        {!!data.annual_checkup && (
          <div style={{ marginTop: 8 }}>
            <TextInput value={data.checkup_package} onChange={v => set('checkup_package', v)} placeholder="体检套餐名称或内容描述" />
          </div>
        )}
      </FieldRow>

      <FieldRow label="计划体检日期">
        <DateInput value={data.checkup_date} onChange={v => set('checkup_date', v)} />
      </FieldRow>
      <FieldRow label="体检机构">
        <SelectInput value={data.checkup_hospital} onChange={v => set('checkup_hospital', v)} options={HOSPITAL_OPTIONS} placeholder="请选择体检机构" />
      </FieldRow>
      <FieldRow label="体检项目">
        <TextInput value={data.checkup_items} onChange={v => set('checkup_items', v)} placeholder="重点关注的体检项目（如：肿瘤标志物、心血管等）" multiline />
      </FieldRow>
      <FieldRow label="备注">
...
export default function AnnualPlanPage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await staffAPI.saveAnnualPlan(id, { planType, moduleData, notes, year })
      toast('年度方案已保存')
      setDirty(false)
    } catch (err) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            ...
            disabled={saving}
            style={{
              background: '#1E6B50', color: '#fff', border: 'none',
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: '20px', marginBottom: 20, border: '1px solid #E0D9CE' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
              {PLAN_TYPES.map(pt => (
                <div
                  key={pt.key}
                  onClick={() => { setPlanType(pt.key); setDirty(true) }}
                  style={{
                ...
                >
                  ...
                  {planType === pt.key && (
                    <span style={{ marginLeft: 'auto', color: pt.color, fontSize: 18 }}>✓</span>
                  )}
                </div>
              ))}
            </div>
