...
export default function PatientDetailPage() {
  ...
  const [execForm, setExecForm] = useState({ type: 'phone', content: '', status: 'completed', serviceChecklist: [], appointmentDetails: {} })
  ...
  const openExec = (f) => {
    ...
    setExecForm({ type: f.type || 'phone', content: '', status: 'completed', serviceChecklist: normalizeCheckupOnsiteChecklist(f, checklist), appointmentDetails: bookingDetailsFromTask(f) })
  }
      ...
      {execItem && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setExecItem(null) }}>
          <div className="modal" style={{ maxWidth: (isCheckupBookingTask(execItem) || isCheckupOnsiteTask(execItem) || isCheckupReportCollectionTask(execItem)) ? 780 : 520 }}>
            <div className="modal-header">
              <h3 className="modal-title">{isCheckupReportCollectionTask(execItem) ? '确认或上传体检报告' : isCheckupBookingTask(execItem) ? '确认体检预约并交接陪诊' : execItem.taskRole === 'supervisor' ? '核对代办结果与检查单' : execItem.taskRole ? '记录事务完成情况' : '执行随访'}</h3>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={handleExec} disabled={execSaving || execItem.isBlocked}>
                {execItem.isBlocked ? '等待上一环节完成' : execSaving ? '保存中...' : isCheckupReportCollectionTask(execItem) ? (execForm.serviceChecklist?.[0]?.collectionStatus === 'complete' ? '完成闭环并进入解析' : '保存报告回收进度') : isCheckupBookingTask(execItem) ? '确认预约并转交陪诊' : execItem.taskRole === 'supervisor' ? '保存督办结论' : execItem.taskRole ? '保存事务记录' : '保存随访结果'}
              </button>
            </div>
          </div>
        </div>
