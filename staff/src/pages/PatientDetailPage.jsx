      ...
      {execItem && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setExecItem(null) }}>
          <div className="modal" style={{ maxWidth: (medicalProxyStage(execItem) || isCheckupBookingTask(execItem) || isCheckupOnsiteTask(execItem) || isCheckupReportCollectionTask(execItem) || isOutpatientAdvisorAssessmentTask(execItem) || isOutpatientAppointmentTask(execItem) || isOutpatientStaffAssignmentTask(execItem) || isOutpatientEscortVisitTask(execItem) || isOutpatientPostVisitReviewTask(execItem)) ? 780 : 520 }}>
            <div className="modal-header">
              <h3 className="modal-title">{medicalProxyStage(execItem) ? (/就医规划/.test(`${execItem.theme || ''} ${execItem.sourceOrderId?.serviceName || ''}`) ? medicalProxyStage(execItem) === 'advisor' ? '就医规划 · 健康顾问提出建议' : '就医规划 · 健康规划师客户沟通与结案' : ({ intake: '医疗代诊 · 核对既有资料', collect: '医疗代诊 · 指导上传并选定资料', audit: '医疗代诊 · 健管审核资料', advisor: '医疗代诊 · 健康顾问确认方案', planner: '医疗代诊 · 健康规划师复核方案', booking: '医疗代诊 · 专家门诊预约', execute: '医疗代诊 · 执行结果' }[medicalProxyStage(execItem)])) : isOutpatientEscortVisitTask(execItem) ? '记录检查及专家门诊陪诊' : isOutpatientPostVisitReviewTask(execItem) ? '查看陪诊资料并制定随访计划' : isOutpatientAppointmentTask(execItem) ? '安排代诊约诊服务' : isOutpatientStaffAssignmentTask(execItem) ? '安排门诊执行人员' : isOutpatientAdvisorAssessmentTask(execItem) ? '健康顾问就医评估' : isCheckupReportCollectionTask(execItem) ? '确认或上传体检报告' : isCheckupBookingTask(execItem) ? '确认体检预约并交接陪诊' : execItem.taskRole === 'supervisor' ? '核对代办结果与检查单' : execItem.taskRole ? '记录事务完成情况' : '执行随访'}</h3>
          ...
          <div className="modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {medicalProxyStage(followUpDetail) === 'supervise' && <div style={{ background: '#EFF8F4', padding: 12, borderRadius: 8, fontSize: 13 }}>
                <strong>健康规划师持续督办</strong><br />当前环节：{({ collect: '指导客户上传并选定资料', audit: '健管专员审核', advisor: '健康顾问确认方案', planner_followup: '顾问建议已完成，待与客户沟通', planner: '规划师复核并预指派就医专员', booking: '健管专员预约专家门诊', appointment_review: '健康顾问重新核对约诊需求', post_visit_audit: '等待就诊后资料及健管审核', followup_review: '健康顾问审核AI随访计划', post_visit_review: '健康顾问查看就诊后资料', execute: '就医专员执行代诊', completed: '服务完成' })[followUpDetail.formData?.currentStage] || '处理中'}<br />服务内容：{followUpDetail.formData?.serviceContent || '-'}<br />客户诉求：{followUpDetail.formData?.customerNeed || '-'}
              </div>}
