...
import OutpatientProxyVisitForm, { emptyOutpatientProxyVisit, isOutpatientProxyVisitTask, validateOutpatientProxyVisit } from '../components/OutpatientProxyVisitForm'
...
const isMedicalProxyMedicationTask = task => /代配药|代取药/.test([
  task?.theme,
  task?.sourceOrderId?.serviceName,
...
const isMedicalEscortTask = task => task?.formData?.medicalEscort === true
  ...
  || /陪同就医|就医陪同|陪同看诊|陪同检查|陪同体检|陪同治疗/.test([
    task?.theme,
    task?.sourceOrderId?.serviceName,
    task?.sourceOrderId?.specificationLabel,
  ].filter(Boolean).join(' '))
...
function DiseaseArchivePanel({ patientId, user, serviceRecords, onSaved, onOpenServiceRecord, toast }) {
  ...
  const emptyCourse = { occurredAt: new Date().toISOString().slice(0, 10), content: '', symptoms: '', examination: '', diagnosis: '', medicationChange: '', treatmentResponse: '', nextPlan: '', sourceType: 'client_report', sourceInstitution: '', sourceDepartment: '', sourceDoctor: '', verificationStatus: 'self_reported' }
  ...
  const summaryRows = [['主诉/主要健康诉求', dossier.summary?.chiefComplaint], ['首次发病与既往经过', dossier.summary?.presentIllness], ['医疗机构检查信息', dossier.summary?.physicalExam], ['相关接触/流行病学信息', dossier.summary?.epidemiologicalHistory], ['医疗机构诊断归档', dossier.summary?.initialDiagnosis], ['医疗机构治疗及用药信息', dossier.summary?.currentMedication]]
  return <>
    ...
    {editingSummary && <div className="modal-overlay" onClick={() => setEditingSummary(false)}><div className="modal" style={{ maxWidth:720 }} onClick={e => e.stopPropagation()}><div className="modal-header"><h3 className="modal-title">专病健康信息摘要</h3><button className="modal-close" onClick={() => setEditingSummary(false)}>×</button></div><div className="modal-body" style={{ display:'grid', gap:10 }}><div><label className="form-label">专病名称 *</label><input className="form-control" value={summaryForm.diseaseName || ''} onChange={e => setSummaryForm(f => ({...f,diseaseName:e.target.value}))} placeholder="如：眩晕（前庭神经炎）" /></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><div><label className="form-label">信息来源 *</label><select className="form-control" value={summaryForm.sourceType} onChange={e=>setSummaryForm(f=>({...f,sourceType:e.target.value}))}>{Object.entries(sourceLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div><div><label className="form-label">核验状态 *</label><select className="form-control" value={summaryForm.verificationStatus} onChange={e=>setSummaryForm(f=>({...f,verificationStatus:e.target.value}))}>{Object.entries(verifyLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div></div>{summaryForm.sourceType !== 'client_report' && <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:10}}><input className="form-control" placeholder="来源医疗机构" value={summaryForm.sourceInstitution} onChange={e=>setSummaryForm(f=>({...f,sourceInstitution:e.target.value}))}/><input className="form-control" placeholder="科室" value={summaryForm.sourceDepartment} onChange={e=>setSummaryForm(f=>({...f,sourceDepartment:e.target.value}))}/><input className="form-control" placeholder="医生" value={summaryForm.sourceDoctor} onChange={e=>setSummaryForm(f=>({...f,sourceDoctor:e.target.value}))}/></div>}{[['chiefComplaint','主诉/主要健康诉求',2],['presentIllness','首次发病与既往经过',5],['physicalExam','医疗机构检查信息',2],['epidemiologicalHistory','相关接触/流行病学信息',2],['initialDiagnosis','医疗机构诊断归档',2],['currentMedication','医疗机构治疗及用药信息',3]].map(([key,label,rows]) => <div key={key}><label className="form-label">{label}</label><textarea className="form-control" rows={rows} value={summaryForm[key] || ''} onChange={e => setSummaryForm(f => ({...f,[key]:e.target.value}))} /></div>)}</div><div className="modal-footer"><button className="btn btn-secondary" onClick={() => setEditingSummary(false)}>取消</button><button className="btn btn-primary" disabled={saving || !summaryForm.diseaseName?.trim()} onClick={saveSummary}>{saving?'保存中…':'保存摘要'}</button></div></div></div>}
    {addingCourse && <div className="modal-overlay" onClick={() => setAddingCourse(false)}><div className="modal" style={{ maxWidth:720 }} onClick={e => e.stopPropagation()}><div className="modal-header"><h3 className="modal-title">记录「{activeName}」健康变化</h3><button className="modal-close" onClick={() => setAddingCourse(false)}>×</button></div><div className="modal-body" style={{ display:'grid', gap:10 }}><div style={{ padding:'9px 12px', background:'#EFF8F3', color:'#1E6B50', borderRadius:8, fontSize:12 }}>记录客户健康变化和外部医疗信息；联系、协调和服务过程记录在“管理记录”。平台不形成诊疗结论。</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><div><label className="form-label">信息来源 *</label><select className="form-control" value={courseForm.sourceType} onChange={e=>setCourseForm(f=>({...f,sourceType:e.target.value}))}>{Object.entries(sourceLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div><div><label className="form-label">核验状态 *</label><select className="form-control" value={courseForm.verificationStatus} onChange={e=>setCourseForm(f=>({...f,verificationStatus:e.target.value}))}>{Object.entries(verifyLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div></div>{courseForm.sourceType !== 'client_report' && <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:10}}><input className="form-control" placeholder="来源医疗机构" value={courseForm.sourceInstitution} onChange={e=>setCourseForm(f=>({...f,sourceInstitution:e.target.value}))}/><input className="form-control" placeholder="科室" value={courseForm.sourceDepartment} onChange={e=>setCourseForm(f=>({...f,sourceDepartment:e.target.value}))}/><input className="form-control" placeholder="医生" value={courseForm.sourceDoctor} onChange={e=>setCourseForm(f=>({...f,sourceDoctor:e.target.value}))}/></div>}<div><label className="form-label">发生日期</label><input type="date" className="form-control" value={courseForm.occurredAt} onChange={e => setCourseForm(f => ({...f,occurredAt:e.target.value}))} /></div><div><label className="form-label">本次健康变化 *</label><textarea className="form-control" rows={4} value={courseForm.content} onChange={e => setCourseForm(f => ({...f,content:e.target.value}))} /></div>{[['symptoms','症状/感受变化'],['examination','医疗机构检查信息'],['diagnosis','医疗机构诊断归档'],['medicationChange','医疗机构用药医嘱归档'],['treatmentResponse','治疗后反馈'],['nextPlan','后续协作事项']].map(([key,label]) => <div key={key}><label className="form-label">{label}</label><textarea className="form-control" rows={2} value={courseForm[key]} onChange={e => setCourseForm(f => ({...f,[key]:e.target.value}))} /></div>)}</div><div className="modal-footer"><button className="btn btn-secondary" onClick={() => setAddingCourse(false)}>取消</button><button className="btn btn-primary" disabled={saving || !courseForm.content.trim()} onClick={saveCourse}>{saving?'保存中…':'追加健康变化'}</button></div></div></div>}
  </>
}
...
export default function PatientDetailPage() {
  ...
  const stopSupplyReminder = async (kind, record) => {
    if (!window.confirm(`确认停止「${record.name}」自动生成后续随访？当前已生成的待办仍会保留。`)) return
    try {
      const result = await staffAPI.saveSupplyReminder(id, kind, record._id, { enabled: false })
      toast(result.message || '已停止自动生成')
  ...
  const [execForm, setExecForm] = useState({ type: 'phone', content: '', status: 'completed', serviceChecklist: [], appointmentDetails: {}, formData: {} })
  ...
  const handleExec = async () => {
    ...
    const isStaffAssignment = isOutpatientStaffAssignmentTask(execItem)
    ...
    if (isCheckupMedicalExecution) {
      ...
      if (error) { toast(error); return }
    } else if (isAdvisorAssessment) {
      const error = validateOutpatientAssessment(execForm.formData)
      if (error) { toast(error); return }
    } else if (isOutpatientAppointment) {
      const error = validateOutpatientAppointment(execForm.formData)
      if (error) { toast(error); return }
    } else if (isStaffAssignment) {
      const error = validateOutpatientStaffAssignment(execForm.formData)
      if (error) { toast(error); return }
    } else if (isProxyVisit) {
      const error = validateOutpatientProxyVisit(execForm.formData)
      if (error) { toast(error); return }
    } else if (isEscortVisit) {
      const error = validateOutpatientEscortVisit(execForm.formData)
      if (error) { toast(error); return }
  ...
  useEffect(() => {
    if (tab === 'followups' && location.state?.openFollowUp) {
      ...
      if (f.sourceType === 'order' && staff?.role === 'healthPlanner') {
        if (/代配药|代取药/.test([f.sourceOrderId?.serviceName, f.sourceOrderId?.specificationLabel, f.sourceOrderId?.note, f.theme].filter(Boolean).join(' '))) {
            ...
            <div className="card-body">
              {data.insuranceCoverage && (() => {
                ...
                return <div style={{ marginBottom: 14, padding: 12, background: '#EEF7F2', border: '1px solid #CDE5D9', borderRadius: 9 }}>
                  ...
                  {policy.serviceManual && Object.entries({
                    internationalOutpatientBooking: '国际/高端门诊预约', appointmentRequiredInfo: '预约所需信息', providerNetworkCheck: '医院网络核验', directBillingEligibility: '直付适用条件', selfPayReimbursementEligibility: '自费后可报销条件', claimMaterials: '报销资料清单', claimProcess: '报销办理流程', claimDeadline: '报案/提交时限', claimSubmissionChannels: '提交渠道', claimFollowUp: '进度查询与补件', emergencyProcedure: '紧急就医流程', cancellationPolicy: '改期/取消规则', escalationContact: '异常升级联系人', sourceReference: '手册依据',
                  }).some(([key]) => policy.serviceManual[key]) && <details style={{ marginTop: 9, padding: 8, background: '#F5F8F6', borderRadius: 6 }}><summary style={{ cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>保险服务手册 · {policy.serviceManual.verificationStatus === 'verified' ? '已核实' : policy.serviceManual.verificationStatus === 'review' ? '待复核' : '资料待补'}</summary><div style={{ display: 'grid', gap: 8, marginTop: 8 }}>{Object.entries({
                    internationalOutpatientBooking: '国际/高端门诊预约', appointmentRequiredInfo: '预约所需信息', providerNetworkCheck: '医院网络核验', directBillingEligibility: '直付适用条件', selfPayReimbursementEligibility: '自费后可报销条件', claimMaterials: '报销资料清单', claimProcess: '报销办理流程', claimDeadline: '报案/提交时限', claimSubmissionChannels: '提交渠道', claimFollowUp: '进度查询与补件', emergencyProcedure: '紧急就医流程', cancellationPolicy: '改期/取消规则', escalationContact: '异常升级联系人', sourceReference: '手册依据',
                  }).filter(([key]) => policy.serviceManual[key]).map(([key, label]) => <div key={key}><b style={{ fontSize: 12 }}>{label}</b><div style={{ whiteSpace: 'pre-wrap', color: '#65776F', fontSize: 12 }}>{policy.serviceManual[key]}</div></div>)}</div></details>}
                  ...
                  {(policy.hospitalBookingRules || []).length > 0 && <details style={{ marginTop: 9, padding: 8, background: '#F5F8F6', borderRadius: 6 }}><summary style={{ cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>医院预约路径（{policy.hospitalBookingRules.length}条）</summary><div style={{ display: 'grid', gap: 8, marginTop: 8 }}>{policy.hospitalBookingRules.map((item, index) => {
                    const routeLabel = { insurer_vendor: '保司指定服务商预约', platform_assisted: '嘉医健管直接预约医院', customer_self: '客户自行预约', confirm: '预约前需核实' }[item.bookingRoute] || '预约前需核实'
                    return <div key={item._id || index} style={{ padding: 8, border: '1px solid #DDE8E2', borderRadius: 6, background: '#fff' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><b style={{ fontSize: 12 }}>{item.hospitalName}{item.campus ? ` · ${item.campus}` : ''}</b><span style={{ fontSize: 11, color: item.verificationStatus === 'verified' ? '#1E6B50' : '#8A5A00' }}>{item.verificationStatus === 'verified' ? '已核实' : item.verificationStatus === 'review' ? '待复核' : '资料待补'}</span></div><div style={{ marginTop: 4, color: '#1E6B50', fontSize: 12, fontWeight: 700 }}>{routeLabel}</div><div style={{ marginTop: 3, whiteSpace: 'pre-wrap', color: '#65776F', fontSize: 12 }}>{[item.vendorName && `受理方：${item.vendorName}`, item.contact && `电话：${item.contact}`, item.bookingEntry && `入口：${item.bookingEntry}`, item.serviceHours && `服务时间：${item.serviceHours}`, item.leadTime && `提前量：${item.leadTime}`, item.requiredInfo && `所需信息：${item.requiredInfo}`, item.notes].filter(Boolean).join('\n') || '具体操作信息待补充'}</div></div>
                  })}</div></details>}
...
function SendMessageModal({ patientId, patientName, serviceBooking, initialOrder, initialDraft, initialBookingForm, planningTask, onFinishPlanning, onPlanningProductPushed, onConfirmBooking, onClose }) {
  ...
  const [checkupPreparation, setCheckupPreparation] = useState('')
  ...
  const updateMedicalReminder = (key, value) => setMedicalReminder(current => ({ ...current, [key]: value }))
  const extractMedicalReminder = async () => {
    setExtractingReminder(true); setBookingError('')
    try {
      const response = await staffAPI.getMedicalReminderDraft(orderId, msgs.slice(-80).filter(message => !message.recalled).map(message => `${message.type === 'user' ? '客户' : message.sender || '服务人员'}：${message.content || message.text || ''}`))
      setMedicalReminder(current => ({ ...current, ...(response.data || {}) }))
    } catch (error) { setBookingError(error.message || 'AI整理失败，请手工填写') }
    finally { setExtractingReminder(false) }
  }
        ...
        {showBookingConfirm && !isMedicalReminderOrder && (
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #E0D9CE', background: '#FFF8ED', display: 'grid', gap: 8, maxHeight: bookingCollapsed ? undefined : '38vh', overflowY: bookingCollapsed ? 'visible' : 'auto', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700 }}>{isCheckupAppointment ? '待约检服务信息确认' : isMedicalReminder ? '复查督办信息确认' : isMedicationProxy ? '代配药信息核对' : '健康顾问约诊建议'} · {order?.serviceName || '服务订单'}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal" style={{ maxWidth: 500 }}>
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <label style={{ display:'flex', alignItems:'center', gap:7, fontSize:13 }}><input type="checkbox" checked={form.requiresConclusion} onChange={e => setForm(f => ({...f, requiresConclusion:e.target.checked}))} />要求接收方提交正式协作反馈</label>
          <div className="form-group" style={{ marginBottom: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
