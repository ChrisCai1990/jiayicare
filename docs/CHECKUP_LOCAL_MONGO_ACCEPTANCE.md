...
- 最新扩展9项通过（8项子测试+总测试），留存库 `jiayicare_checkup_test_79608e80dc3c46c8825f1da332c6f71a`。纯虚构客户及三岗位记录无联系方式、登录凭证或临床资料，不是可登录账号。新增实际服务期门槛、双岗归属和到期拒绝派发验证。
...
并发场景服务期门槛为固定测试输入；新增两项使用实际annualPeriodicGate验证初始服务期有效/到期边界。**本脚本不替代真实续约权限、跨集合更正竞争、浏览器多岗位链路或服务/核销/绩效端到端验收**。这些仍需单独执行并记录。测试不触发AI、付款、客户推送、生产迁移或部署。
