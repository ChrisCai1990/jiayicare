...
2026-09-09 日常操作布局：参考用户提供的企微原生截图，采用浅灰背景、白卡片、蓝色主操作和下划线标签。当前家庭之后直接展示业务内容和新建入口；收件箱、交接概览及快捷指令移到内容下方的折叠工具区，输入指令时自动展开。空列表给出新建指引。只调整展示，不改变权限、绑定及确认写入流程。390px合成浏览器回归检查新建待办进入首屏、辅助输入默认折叠和原有业务操作。

2026-09-09 第二轮密度调整：标题和连接状态同排，服务对象横排，压缩家庭卡片、统计及空列表留白；390px模拟侧栏的待办入口要求在前300px内。普通提示增加关闭按钮，取消绑定编辑清理提示；错误提示不自动隐藏。企微自身顶部工具栏不在本页控制范围内。
