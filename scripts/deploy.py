...
def deploy(backend_only=False, clean=False, github_source=False, skip_data_migrations=False):
    ...
    try:
        ...
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/migrateOutpatientStaleSupervisorsV16.js ] && "
            f"[ ! -f {REPO_DIR}/.outpatient-stale-supervisors-v16-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/migrateOutpatientStaleSupervisorsV16.js && "
            f"touch {REPO_DIR}/.outpatient-stale-supervisors-v16-applied; fi",
            timeout=120,
            label="清理门诊一站式已完成环节遗留的督办任务",
        )
        if code:
        ...
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/migrateOutpatientReportAuditTaskV18.js ] && "
            f"[ ! -f {REPO_DIR}/.outpatient-report-audit-task-v18-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/migrateOutpatientReportAuditTaskV18.js && "
            f"touch {REPO_DIR}/.outpatient-report-audit-task-v18-applied; fi",
            timeout=120,
            label="补齐门诊一站式健管专员资料审核任务",
        )
        if code:
        ...
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/migrateMedicalProxyLegacyOrderSupervisorsV6.js ] && "
            f"[ ! -f {REPO_DIR}/.medical-proxy-legacy-order-supervisors-v6-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/migrateMedicalProxyLegacyOrderSupervisorsV6.js && "
            f"touch {REPO_DIR}/.medical-proxy-legacy-order-supervisors-v6-applied; fi",
            timeout=120,
            label="取消同一客户已被新订单替代的旧版医疗代诊督办",
        )
        if code:
        ...
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/migrateMedicalProxyExecuteV9.js ] && "
            f"[ ! -f {REPO_DIR}/.medical-proxy-execute-v9-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/migrateMedicalProxyExecuteV9.js && "
            f"touch {REPO_DIR}/.medical-proxy-execute-v9-applied; fi",
            timeout=120,
            label="修复医疗代诊执行任务并启用病历与随访交付",
        )
        if code:
            raise RuntimeError("医疗代诊执行任务迁移失败")
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/migrateMedicalProxyProductFlowV10.js ] && "
            f"[ ! -f {REPO_DIR}/.medical-proxy-product-flow-v10-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/migrateMedicalProxyProductFlowV10.js && "
            f"touch {REPO_DIR}/.medical-proxy-product-flow-v10-applied; fi",
            timeout=120,
            label="统一 Admin 医疗代诊商品与商城、医护端服务流程",
        )
        if code:
        ...
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/migrateExpertAppointmentWorkflowV12.js ] && "
            f"[ ! -f {REPO_DIR}/.expert-appointment-workflow-v12-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/migrateExpertAppointmentWorkflowV12.js && "
            f"touch {REPO_DIR}/.expert-appointment-workflow-v12-applied; fi",
            timeout=120,
            label="创建专家约诊方案并关闭已完成约诊订单",
        )
        if code:
        ...
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/cleanupExpertAppointmentReviewV15.js ] && "
            f"[ ! -f {REPO_DIR}/.expert-appointment-review-cleanup-v15-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/cleanupExpertAppointmentReviewV15.js && "
            f"touch {REPO_DIR}/.expert-appointment-review-cleanup-v15-applied; fi",
            timeout=120,
            label="清理非指定客户的历史专家约诊复核任务",
        )
        if code:
        ...
        code, _ = run_migration(
            f"if [ -f {REPO_DIR}/backend/src/scripts/renameExpertAppointmentReturnV17.js ] && "
            f"[ ! -f {REPO_DIR}/.expert-appointment-return-v17-applied ]; then "
            f"cd {REPO_DIR}/backend && node src/scripts/renameExpertAppointmentReturnV17.js && "
            f"touch {REPO_DIR}/.expert-appointment-return-v17-applied; fi",
            timeout=120,
            label="将专家约诊任务明确为回退完善新增类目",
        )
        if code:
