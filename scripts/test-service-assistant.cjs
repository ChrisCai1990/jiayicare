...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button',{name:'待归档',exact:true}).click();
    await page.getByText('等待会话存档审核及采集接入；目前不会自动收到真实群图片。',{exact:true}).waitFor();
    await page.screenshot({path:path.join(output,'material-inbox-waiting.png'),fullPage:true});
    fixture.models.ServiceGroup.rows[0].archiveConsent=true;
    const bytes=Buffer.from('89504e470d0a1a0a0000000d49484452','hex');
    const stored=await require('../backend/src/utils/oss').uploadBuffer(bytes,'image/png','service-group-staging');
    fixture.models.ServiceGroupMessage.seed({groupId:fixture.ids.group,messageId:'synthetic-photo',sender:'测试家属',sentAt:new Date(),expiresAt:new Date(Date.now()+86400000),sealedText:'synthetic',attachment:{name:'测试打卡.png',ossKey:stored.key,mimeType:'image/png',size:bytes.length,sha256:require('crypto').createHash('sha256').update(bytes).digest('hex')}});
    const inboxRefresh=page.waitForResponse(r=>r.url().includes('/by-chat/'));
    await page.evaluate(()=>document.dispatchEvent(new Event('visibilitychange')));await inboxRefresh;
    await page.getByLabel('选择资料 测试打卡.png').check();
    await page.getByLabel('资料所属成员').selectOption(fixture.ids.patient);
    await page.getByLabel('归档用途').selectOption('checkin');
    await page.getByLabel('本批资料名称').fill('测试打卡原图');
    await page.getByLabel('资料日期').fill('2026-09-09');
    await page.screenshot({path:path.join(output,'material-inbox-confirm.png'),fullPage:true});
    await page.getByRole('button',{name:'确认归档所选资料',exact:true}).click();
    await page.getByText('1份完成（其中0份为已有原件），0份未完成。',{exact:true}).waitFor();
    if(!fixture.models.ServiceRecord.rows.some(r=>r.title==='打卡原图 · 测试打卡原图'))throw new Error('Photo confirmation missing service record');
