...
async function main() {
  ...
  try {
    ...
    await page.getByLabel('客户名或报告名').fill('.*');
    await page.getByRole('button', { name: '查询', exact: true }).click();
    await page.getByText('没有符合条件的调用记录。').waitFor();
    await page.getByLabel('客户名或报告名').fill('搜索测试客户 体检');
    await page.getByLabel('客户名或报告名').press('Enter');
    await page.getByRole('cell', { name: /用量待核对/ }).waitFor();
